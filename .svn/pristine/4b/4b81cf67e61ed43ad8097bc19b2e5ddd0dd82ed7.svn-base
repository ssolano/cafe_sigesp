...
/*		$ls_empresa=$this->is_empresa["codemp"];
	...
	function uf_insert_tipoestructura(&$as_codigo,$as_tipo,$as_descripcion,$aa_seguridad)
	{
		...
		{			
			...
			{
				$this->io_msg->message("CLASE->TipoEstructura MÉTODO->uf_insert_tipoestructura ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			}
		}
		else
		{
	...
	function uf_update_tipoestructura($as_codigo,$as_tipo,$as_descripcion,$aa_seguridad)
	{
		...
		if($li_row===false)
		{			
			$this->io_msg->message("CLASE->TipoEstructura MÉTODO->uf_update_tipoestructura ERROR->".$this->io_function->uf_convertirmsg($this->io_sql->message)); 
			$this->io_sql->rollback();
		}
		else
		{
