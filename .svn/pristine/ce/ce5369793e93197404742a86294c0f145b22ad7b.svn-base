...
if ($ls_operacion=="ue_ver_personal_sueldos")
{
  $lb_hay = $lo_dao->get_personal_sueldos($_GET["codper"],$_GET["nomper"],$_GET["apeper"],"ORDER BY codper ASC",$la_datos);
...
else
{
  ...
  $lb_hay = $lo_dao->get_personal_sueldos($ls_codper, $ls_nomper, $ls_apeper, "ORDER BY $sort_col $sort_dir LIMIT $offset,$pagesize",$la_datos); 
  if ($lb_hay)
  {
    $li_registros = (count($la_datos,COUNT_RECURSIVE)/count($la_datos))-1;  
