			...
			{  
			   case "":
			    ...
			    case "cuenta":
					...
					{
						print "<tr class=celdas-azules>";
						print "<td style=text-align:center width=100><a href=\"javascript: aceptar2('$cuenta','$denominacion','$status');\">".$cuenta."</a></td>";
						print "<td style=text-align:left width=400>".$denominacion."</td>";
					}
					print "</tr>";	
				break;
...
fop = opener.document.formulario;
  ...
  {
    fop.txtcuentaplan.value=cuenta;
	fop.txtdencuentaplan.value=d;
	close();
  }
  
  function aceptar2(cuenta,denominacion,status)
  {
    fop.txtcuenta.value=cuenta;
	fop.txtdencuenta.value=denominacion;
	close();
  }
  ...
  function ue_search()
  {
	  ...
	  f.action="sigesp_catdinamic_ctas.php?cuenta=<?PHP print $ls_cuenta;?>";
	  f.submit();
  }
</script>
</html>