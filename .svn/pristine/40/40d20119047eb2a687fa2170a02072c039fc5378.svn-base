...
require_once("class_folder/sigesp_rpc_c_comunidad.php");
...
$io_comunidad = new sigesp_rpc_c_comunidad($conn);//Instanciando la Clase Sigesp Definiciones.
...
//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////////////////
	...
	$ls_sistema  = "CFG";
	$ls_ventanas = "sigesp_rpc_d_comunidad.php";
...
if ($ls_operacion=="NUEVO") 
   {
	 $ls_codigo = $io_comunidad->uf_generar_codigo($lb_empresa,$ls_codemp,'sigesp_comunidad','codcom',$ls_codpais,$ls_codest,$ls_codmun,$ls_codpar);
	 if (empty($ls_codigo))
	    {
      ...
      if ($lb_existe)
	     {
	       ...
	       $lb_tiene     = $io_chkrel->uf_check_relaciones($ls_codemp,$ls_condicion,'sigesp_comunidad',$ls_codigo,$ls_mensaje);//Verifica los movimientos asociados a la cuenta
           if (!$lb_tiene)
		      {
			        ...
			        {   
			          ...
			          if (campo_requerido(txtdenominacion,"La Denominación de la Comunidad debe estar llena !!")==false)
				      ...
				      else
				         {
					       ...
					       f.action          = "sigesp_rpc_d_comunidad.php";
					       f.submit();
					     }
			        }
		       }			
	...
	if (li_leer==1)
	   {
         if (f.cmbparroquia.value=="---")
            ...
            {   
			  ...
			  pagina            = "sigesp_cfg_cat_comunidad.php?hidpais="+ls_pais+"&hidestado="+ls_estado+"&hidmunicipio="+ls_municipio+"&hidparroquia="+ls_parroquia;
