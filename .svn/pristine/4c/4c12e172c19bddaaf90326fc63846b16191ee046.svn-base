...
//ini_set('display_errors','On');
	...
	function uf_print_detalle($la_data,&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('nro'=>array('justification'=>'center','width'=>30), // Justificación y ancho de la columna
						 			   ...
						 			   'ubifis'=>array('justification'=>'center','width'=>100),
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('nro'=>array('justification'=>'center','width'=>30), // Justificación y ancho de la columna
						 			   ...
						 			   'cuenta'=>array('justification'=>'center','width'=>100), // Justificación y ancho de la columna
                                                                           'ubifis'=>array('justification'=>'left','width'=>100), // Justificación y ancho de la columna
						 			   'monto'=>array('justification'=>'right','width'=>50))); // Justificación y ancho de la columna
	...
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_print_piecabecera($ai_total,&$io_pdf,$ai_nro_total_personal=0)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'width'=>500); // Ancho Máximo de la tabla
		$io_pdf->ezTable($la_data,$la_columna,'',$la_config);	
		$la_data=array(array('titulo'=>'<b>Cantidad de Personal: </b>','neto'=>$ai_nro_total_personal." persona(s)"),array('titulo'=>'<b>Total General '.$ls_bolivares.': </b>','neto'=>$ai_total));
	...
	else // Imprimimos el reporte
	{
		...
		$li_total=0;
		$li_nro_total=0; //Total del personal en el reporte
                
                                                         //====================================================================
                    //Buscamos las ubicaciones fisicas
                    $ubicaciones_valido = $io_report->uf_ubicaciones_fisicas($ls_codban,$ls_suspendido,$ls_tipcueban,$ls_quincena,$ls_subnomdes,$ls_subnomhas,
		...
		while(!$io_report->rs_data->EOF)
		{
               ...
               foreach($io_report->rs_data_ubicaciones as $ubic){                                                                                                  //
                   //echo "<pre>". print_r($io_report->rs_data_ubicaciones,true)."</pre>"; die();
                    $ls_bcodubifis=$ubic["codubifis"];
                    //echo $ls_bcodubifis . "***<br/>";
                    //Escribimos el nombre de la ubicacion fisica
                    $io_pdf->ezText("\n\n\xbb".$ubic["desubifis"]."\n",10,array());
                    $li_nro=0; //Numero de personal para esta ubicacion fisica
                
			
//                    
//               $ubica = array('0045','0048');
//               foreach ($ubica as $u){
//                        $ls_bcodubifis = $u; 
			...
			if (($ls_tipocuenta == '') ||  ($ls_tipocuenta == $ls_tipcueban))
			{
				...
				$lb_valido=$io_report->uf_listadobanco_personal($ls_codban,$ls_suspendido,$ls_tipcueban,$ls_quincena,$ls_subnomdes,$ls_subnomhas,
																$ls_codperdes,$ls_codperhas,$ls_orden,$ls_bcodubifis); // Obtenemos el detalle del reporte
				if($lb_valido)
				{
					$li_subtot=0;				
					...
					while(!$io_report->rs_data_detalle->EOF)
					{
						...
						$ls_codcueban=$io_report->rs_data_detalle->fields["codcueban"];
                                                $ls_ubifis = $io_report->rs_data_detalle->fields["desubifis"];
						$la_data[$li_s]=array('nro'=>$li_nro,'cedula'=>$ls_cedper,'nombre'=>$ls_nomper,'monto'=>$li_monnetres, 'cuenta'=>$ls_codcueban, 'ubifis'=>$ls_ubifis);
						$li_s++;
			...
			$lb_valido=$io_report->uf_listadobancotaquilla_personal($ls_codban,$ls_suspendido,$ls_quincena,$ls_subnomdes,$ls_subnomhas,$ls_codperdes,$ls_codperhas,$ls_orden,$ls_bcodubifis); // Obtenemos el detalle del reporte
			if($lb_valido)
			{
				$li_subtot=0;				
				$li_totrow_det=$io_report->rs_data_detalle->RecordCount();				
				...
				while(!$io_report->rs_data_detalle->EOF)
				{
					...
					$ls_codcueban=$io_report->rs_data_detalle->fields["codcueban"];
                                        $ls_ubifis = $io_report->rs_data_detalle->fields["desubifis"];
					$la_data[$li_s]=array('nro'=>$li_nr,'cedula'=>$ls_cedper,'nombre'=>$ls_nomper,'monto'=>$li_monnetres, 'cuenta'=>$ls_codcueban,'ubifis'=>$ls_ubifis);
					$li_s++;
					$io_report->rs_data_detalle->MoveNext();
				}
				if(!empty($la_data))
