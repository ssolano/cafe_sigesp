...
/******************************************************************************************************************/		
		...
		if($as_tiporet=="I")
		 {
		   if($as_tipo=='P')
		    {
  		     $ls_sql = " SELECT  RD.cod_pro,MAX(RDD.codded) AS codded,MAX(RDD.numrecdoc) AS numrecdoc,MAX(PRO.nompro)".
			           " AS	nompro,MAX(PRO.dirpro) AS dirpro,MAX(PRO.rifpro) AS rifpro".
					   ...
					   " AND (SD.iva=1) AND (SD.codded=RDD.codded) AND (RDD.numrecdoc=RD.numrecdoc) AND".
		    ...
		    else
		    {
			 ...
			 $ls_sql =  " SELECT RD.ced_bene as cod_pro,MAX(RDD.codded) AS codded,MAX(RDD.numrecdoc) AS numrecdoc,".$ls_parametro.",MAX(BEN.dirbene) as dirpro,MAX(BEN.rifben) as rifpro".
	...
	function uf_get_documento($as_mes,$as_agno,$as_codprobene,$as_tipo,$as_tiporet,&$as_RD)
	{
	   //////////////////////////////////////////////////////////////////////////////
	   ...
	   if($as_tiporet=='I')
        {	   	   
	      if($as_tipo=='P')
		   {
			$ls_sql = " SELECT ".$ls_id." as id,RD.numrecdoc,RD.numrecdoc,RD.fecemidoc,SUM(RDC.monobjret) AS basimpiva,".
...
/******************************************************************************************************************/
	...
	function uf_procesar_cmp_retencion($as_mes,$as_agno,$as_probendesde,$as_probenhasta,$as_tipo,$as_tiporet,&$aa_numcmp,$aa_seguridad)
	{
		...
		for($li_i=1;$li_i<=$li_totalfilaspro;$li_i++)
		{
			...
			$this->uf_get_documento($as_mes,$as_agno,$ls_codpro,$as_tipo,$as_tiporet,&$la_RD);
