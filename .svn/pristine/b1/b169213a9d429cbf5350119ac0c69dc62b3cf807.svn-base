...
class sigesp_snorh_c_metodo_cestaticket
{
									 ...
									 $as_conceptocero,$as_orden)
	{
		...
		$ls_sql="SELECT sno_personal.codper, MAX(sno_personal.cedper) AS cedper, MAX(sno_personal.nacper) AS nacper,  ".
				"		MAX(sno_personal.nomper) AS nomper, MAX(sno_personal.apeper) AS apeper, MAX(sno_personal.fecnacper) as fecnacper, ".
				...
				"   AND sno_cestaticunidadadm.prouniadm = sno_unidadadmin.prouniadm ".
                                "   GROUP BY sno_unidadadmin.desuniadm, sno_personal.codper,sno_personal.apeper,sno_personal.nomper,sno_personal.cedper ".
                                //"   ORDER BY sno_unidadadmin.desuniadm, sno_personal.codper ";
				//" GROUP BY sno_personal.codper,sno_personal.apeper,sno_personal.nomper,sno_personal.cedper ".
				$ls_orden."";
		$rs_data=$this->io_sql->select($ls_sql);
		//echo $ls_sql; die(); //CAMB //DEBUG
	...
	function uf_listado_gendisk2($as_codnomdes,$as_codnomhas,$as_ano,$as_mes,$as_codperi,$as_codcestic,$as_conceptocero,$as_orden)
	{
		...
		$ls_sql="SELECT sno_personal.codper, MAX(sno_personal.cedper) AS cedper, MAX(sno_personal.nacper) AS nacper,  ".
				...
				"   ".$ls_orden;
		$rs_data=$this->io_sql->select($ls_sql);
		//echo $ls_sql; die(); //CCAMB DIE //DEBUG?
		if($rs_data===false)
		{
	...
	{	
	  	...
	  	switch ($as_metodo)
		{
			...
			case "Banco de Venezuela Alimentacion":
			
				$lb_valido=$this->uf_metodo_banco_venezuela_alimentacion($as_ruta,$as_anocurper,$as_mescurper,$aa_ds_cestaticket,
											   						     $as_codcli,$as_codprod,$as_punent,$ad_fecha);
				break;
	...
	function uf_metodo_accord_tarjeta($as_ruta,$as_anocurper,$as_mescurper,$aa_ds_cestaticket,$as_codcli,$as_codprod,$as_punent,$ad_fecha)
	{
		...
		if($li_count>0)
		{
			$ls_fecha=str_replace("-","/",$ad_fecha);
                        ...
                        $ls_nombrearchivo=$as_ruta."/T_".$as_codcli."_".$as_codprod."_".date("dmY")."_01.txt";
			...
			else
			{
				...
				$lb_valido = false;
                        ...
                        $ls_gerencia=$aa_ds_cestaticket->data["desuniadm"][1];
                        if ($ls_creararchivo)  //Chequea que el archivo este abierto
                        {
                                if (@fwrite($ls_creararchivo,$ls_gerencia."\r\n")===false)//Escritura
                                {
                                        $this->io_mensajes->message("No se puede escribir el archivo ".$ls_nombrearchivo);
                                        $lb_valido = false;
                                }
			...
			for($li_i=1;$li_i<=$li_count;$li_i++)
			{
				if ($ls_gerencia!=$aa_ds_cestaticket->data["desuniadm"][$li_i]){
                                    if ($ls_creararchivo)  //Chequea que el archivo este abierto
                                    {
                                            if (@fwrite($ls_creararchivo,$aa_ds_cestaticket->data["desuniadm"][$li_i]."\r\n")===false)//Escritura
                                            {
                                                    $this->io_mensajes->message("No se puede escribir el archivo ".$ls_nombrearchivo);
                                                    $lb_valido = false;
                                            }
                                ...
                                $ls_gerencia=$aa_ds_cestaticket->data["desuniadm"][$li_i];
				...
				$ls_cedper=$this->io_funciones->uf_cerosizquierda($ls_cedper,8);
                                $nombre=split(" ",trim($aa_ds_cestaticket->data["nomper"][$li_i]));
                                $apellido=split(" ",trim($aa_ds_cestaticket->data["apeper"][$li_i]));
                                $ls_nombreper=$nombre[0];
				$ls_apellidoper=$apellido[0];
                                $ls_fecnacper= $aa_ds_cestaticket->data["fecnacper"][$li_i];
                                $ls_fecnacper=$this->io_funciones->uf_convertirfecmostrar($ls_fecnacper); // fecha de nacimiento DD/MM/YYYY
				...
				//$ld_monto=$this->io_funciones->uf_cerosizquierda(number_format($ld_monto,0,".",""),15);
                                $ld_monto=number_format($aa_ds_cestaticket->data["monto"][$li_i],2,",","");       //Monto aporte
				$ls_cadena=$ls_nacper.$ls_cedper.";".$ls_nombreper.";".$ls_apellidoper.";".$ls_fecnacper.";".$as_punent.";".$as_codcli.";".$as_codprod.";3;".$ld_monto.";".$ls_fecha."\r\n";
				if ($ls_creararchivo)  //Chequea que el archivo este abierto
				{
					if (@fwrite($ls_creararchivo,$ls_cadena)===false)//Escritura
					{
		...
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_origen=$as_ruta."/accord_tarjeta.txt";
		$ls_codorgcestic=rtrim($aa_ds_cestaticket->data["codorgcestic"][1]);
		$ld_fecproc=str_replace("/","",$ad_fecha);
		$ls_destino=$as_ruta."/c_".$ls_codorgcestic."_".$as_codprod."_".$ld_fecproc."_01.txt";
		copy($ls_origen,$ls_destino);
		chmod($ls_destino,0777);
		$fname = fopen ($ls_destino,"r+");
		$workbook = &new writeexcel_workbook($fname);
	...
	//----------------------------------------------------------------------------------------------------------------------------------
	//-----------------------------------------------------------------------------------------------------------------------------------
	function uf_metodo_banco_venezuela_alimentacion($as_ruta,$as_anocurper,$as_mescurper,$aa_ds_cestaticket,$as_codcli,$as_codprod,
													$as_punent,$ad_fecha)
	{
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//	     Function: uf_metodo_banco_venezuela_alimentacion
		//		   Access: private 
		//	    Arguments: as_ruta  // ruta 
		//                 as_anocurper //  Año en curso
		//                 as_mescurper //  Mes en Curso
		...
		//                 ad_fecha // Fecha de Entrega
		//	  Description: genera el archivo txt a disco para  el banco Industrial para pago de Cesta Ticket
		//	   Creado Por: Ing. Carlos Morales ca.moralb@gmail.com
		// Fecha Creación: 10/02/2015 								
		// Modificado Por: 														Fecha Última Modificación : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		
		//echo "<pre>".print_r($aa_ds_cestaticket,true)."</pre>"; die(); //CAMB //DEBUG
		$lb_valido=true;
		$li_count=$aa_ds_cestaticket->getRowCount("codper");
		if($li_count>0)
		{	
			$ls_nombrearchivo=$as_ruta."/BCO_VENEZUELA_ALIMENTACION_RECARGA_".$as_anocurper."_".$as_mescurper.".txt";
			//Chequea si existe el archivo.
			if (file_exists("$ls_nombrearchivo"))
			{
				if(@unlink("$ls_nombrearchivo")===false)//Borrar el archivo de texto existente para crearlo nuevo.
			...
			//Formateamos fecha:
			$ad_fecha_f = substr($ad_fecha, 0,2)."/".substr($ad_fecha, 3,2)."/".substr($ad_fecha, 8,2);

			//Vamos a armar la cabecera del archivo para abonos
			$ls_identificador = "H"; //(1-1)
			$ls_nomemp = substr(strtoupper($_SESSION["la_empresa"]["nomres"]),0,40);
			$ls_nomemp = str_pad(trim($ls_nomemp),40," ",STR_PAD_RIGHT);
			$ls_nro_empresa = "01020552290000060150";
			$ls_nro_control_consec = "01";
			$ls_fec_efec_pago = $ad_fecha_f;
			$ls_monto_total = str_pad(($aa_ds_cestaticket->data["montototal"][1]*100),13,"0",STR_PAD_LEFT);
			$ls_central = "03291";
			$ls_3_espacios = "   ";
			$ls_codigo_banco = "0102";
			//$ls_nro_lote = "---------------"; //Revisar! cambia en cada TXT //DEBUG //CAMB
			$ls_nro_lote = "600232300".$as_anocurper.$as_mescurper; //Numero de lote. Numero generado para evitar duplicidad de archios procesados
			$ls_rif = str_replace("-","",$_SESSION["la_empresa"]["rifemp"]);
			$ls_total_nro_detalle = str_pad($li_count,7,"0",STR_PAD_LEFT);
			$ls_tipo_nomina = "P"; //Se confirma que siempre se envia letra P (Parcial) y no T (total)
			$ls_codigo_plan = "00600232";
			$ls_fec_generacion = date("d/m/y");
			$ls_hora_generacion = date("H:i:s");
			$ls_codigo_respuesta = "00";
			$ls_nom_archivo = "RECARGA";
			$ls_filler = str_pad("",17," ");
			//somaV
			//$ls_codorgcestic=$this->io_funciones->uf_trim($aa_ds_cestaticket->data["codorgcestic"][1]);
			//$ls_codorgcestic=str_pad($ls_codorgcestic,3,"0",0);
			//$li_totreg=str_pad($li_count,6,"0",0);
			...
			//$ad_fecha=$this->io_funciones->uf_convertirdatetobd($ad_fecha);
			//$ad_fecha=str_replace("-","",$ad_fecha);
			$ls_cadena=$ls_identificador.$ls_nomemp.$ls_nro_empresa.$ls_nro_control_consec.$ls_fec_efec_pago.$ls_monto_total.$ls_central.$ls_3_espacios.$ls_codigo_banco.$ls_nro_lote.$ls_rif.$ls_total_nro_detalle.$ls_tipo_nomina.$ls_codigo_plan.$ls_fec_generacion.$ls_hora_generacion.$ls_codigo_respuesta.$ls_nom_archivo.$ls_filler."\r\n";
			if ($ls_creararchivo)  //Chequea que el archivo este abierto				
			{
				if (@fwrite($ls_creararchivo,$ls_cadena)===false)//Escritura
				{
			...
			{   	
				...
				$ls_id_registro = "1";
				$ls_num_tarjeta = str_pad("",20," "); //No se envia el numero de tarjeta
				$ld_monto=($aa_ds_cestaticket->data["monto"][$li_i]*100);       //Monto aporte
				$ld_monto=$this->io_funciones->uf_cerosizquierda(number_format($ld_monto,0,".",""),11);
				$ls_filler_detalle1 = "    ";
				$ls_nombre = strtoupper(trim($aa_ds_cestaticket->data["nomper"][$li_i] . " " . trim($aa_ds_cestaticket->data["apeper"][$li_i])));	
				$ls_nombre = str_replace(array('Á','É','Í','Ó','Ú','Ñ','\''),array('A','E','I','O','U','N',''),str_pad(substr($ls_nombre,0,40),40," ",STR_PAD_RIGHT));
				$ls_nacper=strtoupper($this->io_funciones->uf_trim($aa_ds_cestaticket->data["nacper"][$li_i]));     //nacionalidad
				$ls_cedper=$this->io_funciones->uf_trim($aa_ds_cestaticket->data["cedper"][$li_i]);     //cedula
				$ls_cedper=$this->io_funciones->uf_cerosizquierda($ls_cedper,9);
				$ls_central_detalle = "003291";
				$ls_codigo_banco = "0102";
				$ls_nro_lote_detalle = $ls_nro_lote;
				$ls_rif_detalle = $ls_rif;
				$ls_codigo_resp_detalle = "0000";
				$ls_filler_detalle2 = str_pad("",55," ");				
				
			
				$ls_cadena=$ls_id_registro.$ls_num_tarjeta.$ld_monto.$ls_filler_detalle1.$ls_nombre.$ls_nacper.$ls_cedper.$ls_central_detalle.$ls_codigo_banco.$ls_nro_lote_detalle.$ls_rif_detalle.$ls_codigo_resp_detalle.$ls_filler_detalle2."\r\n";
				if ($ls_creararchivo)  //Chequea que el archivo este abierto				
				{
					if (@fwrite($ls_creararchivo,$ls_cadena)===false)//Escritura
					{
		...
		return $lb_valido;
	}// end function uf_metodo_banco_venezuela_alimentacion
	//-----------------------------------------------------------------------------------------------------------------------------------
