<?PHP
	...
	function uf_print_encabezado_pagina($as_numanacot,$observacion,$ad_fecha,&$io_pdf)
	{
		...
		$io_pdf->addJpegFromFile('../../shared/imagebank/cintillo_vycs.jpg',25,535,745,65); // Agregar Logo
		$li_tm=$io_pdf->getTextWidth(14,"<b>Análisis de Cotizaciones</b>");
		$tm=396-($li_tm/2);
		$io_pdf->addText($tm,525,14,"<b>"."<b>Análisis de Cotizaciones</b>"."</b>"); // Agregar el título
		$io_pdf->addText(690,525,8,$ad_fecha); // Agregar Numero de la solicitud
		...
		$io_pdf->addText(45,520,8,'<b>Nro.:</b>'.$as_numanacot); // Agregar Numero de la solicitud
		$io_pdf->addText(45,510,8,'<b>Observación:</b>'.$observacion); // Agregar Numero de la solicitud
		$io_pdf->restoreState();
		$io_pdf->closeObject();
