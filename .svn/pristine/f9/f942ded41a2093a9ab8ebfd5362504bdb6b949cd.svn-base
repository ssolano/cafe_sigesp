            ...
            switch(gettype($var)) {
                ...
                case 'string': // STRINGS ARE EXPECTED TO BE IN ASCII OR UTF-8 FORMAT
                    ...
                    for($c = 0; $c < $strlen_var; $c++) {
                        ...
                        // La siguiente condicion  y su contenido fue modificada
                        // por el Ing. Edgar M. Pastran C.
						elseif((($ord_var_c & 0xE0) == 0xC0) or
						         (($ord_var_c & 0xF0) == 0xE0) or
								 (($ord_var_c & 0xF8) == 0xF0)){						           
                            // characters U-00000080 - U-000007FF, mask 110XXXXX, see http://www.cl.cam.ac.uk/~mgk25/unicode.html#utf-8                            
                            $aux = $c;
                            if ($c+1 < $strlen_var)
                            {$aux += 1;}
                            $char = pack('C*', $ord_var_c, ord($var{$aux})); 
							$c+=1;
							if ($c >= $strlen_var)
							{$ascii .= substr($char,0,1);}
							else
                            {$ascii .= $char;}
