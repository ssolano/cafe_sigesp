...
if ($ls_operacion=="CARGAR" )
{
 ...
 $ls_denominacion=$rs_load->fields["denominacion"];
		...
		$ld_asignado=number_format($rs_load->fields["asignado"],2,",","."); //2
                //$ld_asignado=$rs_load->fields["asignado"];
		...
		$object[$i][3]="<input type=text name=txtAsignacion".$i." class=sin-borde onBlur='uf_formato(this)' onKeyPress='return keyRestrictgrid(event)'   value=$ld_asignado  onFocus= uf_fila(".$i.") style=text-align:right>"; //onKeyUp='ue_validarcomas_puntos(this)'  para validar caracteres especiales , puntos y comas 
...
{ 
   ...
   { 
	 ...
	 if($lb_valido)
	 {        //Llamado a otra operacion.... REVISAR
