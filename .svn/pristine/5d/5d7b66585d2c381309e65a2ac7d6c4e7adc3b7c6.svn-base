	...
	function uf_balance_general($ad_fecfin,$ai_nivel)
	{
	 ...
	 else
	 {
		...
		while($row=$this->io_sql->fetch_row($rs_data))
		{
		  ...
		  $ls_cadena=substr($ls_sc_cuenta,0,1);
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//       Function : uf_cuenta_resultado
//		    Acess : private
//	    Arguments : $ad_fecfin
//    Description : función que determina el total del resultado del ejercicio 
//	   Creado Por : Ing. Erlinda Tovar
// Fecha Creación : 12/08/2013
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////  
	
function uf_cuenta_resultado($ad_fecfin)
	{
		$lb_valido=true;
		$ld_montocierre=0;
		$ad_fecfin=$this->io_fun->uf_convertirdatetobd($ad_fecfin);
	    ...
	    {			
		     $ls_sql="SELECT SC.sc_cuenta,SC.denominacion,SC.status,SC.nivel as rnivel, 
                             ...
                             0 as nivel 
                      FROM scg_cuentas SC 
                      where  ( SC.sc_cuenta like '".$ls_ctaresult."%') AND SC.nivel = 1
                      ORDER BY SC.sc_cuenta ";
		}	
		else
		{
		 $ls_sql=" SELECT SC.sc_cuenta,SC.denominacion,SC.status,SC.nivel as rnivel, ".
              ...
              " ON SC.sc_cuenta=curSaldo.sc_cuenta ".
              " WHERE SC.codemp=curSaldo.codemp AND  curSaldo.codemp='".$ls_codemp."' AND ".
			  "        ( SC.sc_cuenta like '".$ls_ctaresult."%') ".
			  " AND SC.nivel = 1".
              " ORDER BY trim(SC.sc_cuenta) ";  
		}
	    //print $ls_sql;
	    ...
	    if($rs_data===false)
	    {// error interno sql
			$this->is_msg_error="Error en consulta metodo uf_cuenta_resultado ".$this->io_fun->uf_convertirmsg($this->io_sql->message);
			//print $this->io_sql->message;
			$lb_valido = false;
		}
		else
		{
			...
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				$ld_montocierre=$row["total_debe"];
			}//while
			$lb_valido = true;
		} //else
		return $ld_montocierre;
    }
 
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
//       Function : uf_total_activo
//		    Acess : private
//	    Arguments : $ad_fecfin
//    Description : función que determina el total de los activos
//	   Creado Por : Ing. Erlinda Tovar
// Fecha Creación : 12/08/2013
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////   

 function uf_total_activo($ad_fecfin)
	{
		$this->ls_activo=trim($this->la_empresa["activo"]);
		$lb_valido=true;
		$ld_monto_debe=0;
	    ...
	    {			
		     $ls_sql="SELECT SC.sc_cuenta,SC.denominacion,SC.status,SC.nivel as rnivel, 
                             ...
                             0 as nivel 
                      FROM scg_cuentas SC 
                      where  ( SC.sc_cuenta like '".$this->ls_activo."%') AND SC.nivel = 1
                      ORDER BY SC.sc_cuenta ";
		}	
		else
		{
	    ...
	    if($rs_data===false)
	    {// error interno sql
			$this->is_msg_error="Error en consulta metodo uf_total_activo ".$this->io_fun->uf_convertirmsg($this->io_sql->message);
			//print $this->io_sql->message;
			$lb_valido = false;
		}
		else
		{
			while($row=$this->io_sql->fetch_row($rs_data))
			{
				$ld_monto_debe=$row["total_debe"];
				$ld_monto_haber=$row["total_haber"];
			}//while
			$ld_monto_activo=$ld_monto_debe-$ld_monto_haber;
			$lb_valido = true;
		} //else
		return $ld_monto_activo;
    }
