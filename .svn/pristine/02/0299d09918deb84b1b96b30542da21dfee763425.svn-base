...
class sigesp_sno_c_vacacion
{
	...
	function uf_load_personalporvencer($ad_codperdes,$ad_codperhas,&$rs_data)
	{
		...
		$ls_sql="SELECT sno_personalnomina.codper ".
				...
				"   AND sno_personalnomina.staper='1'".
				"   AND sno_personal.estper='1'".
				"   AND SUBSTR(sno_personal.fecingper::text,1,4)<'".$ls_ano."'".
				"   AND SUBSTR(sno_personal.fecingper::text,6,2)='".$ls_mes."'".
				"   AND SUBSTR(sno_personal.fecingper::text,9,2)>='".$ls_diades."'".
				"   AND SUBSTR(sno_personal.fecingper::text,9,2)<='".$ls_diahas."' ".
	...
	{	
		...
		else
		{
			...
			$ls_sql="SELECT COALESCE(sum(valsal),0.00) as total ".
					...
					"   AND SUBSTR(sno_hperiodo.fechasper::text,6,2) ='".$ls_mesant."' ".
					"   AND SUBSTR(sno_hperiodo.fechasper::text,1,4) ='".$ls_anoant."' ".
