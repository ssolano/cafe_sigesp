...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function uf_print_encabezado_pagina($as_estcondat,$as_numordcom,$ad_fecordcom,$as_coduniadm,$as_denuniadm, $as_codfuefin,
									   ...
									   $ls_forpagcom,$as_nomusuario,$as_fechentdesde,$as_fechenthasta,&$io_pdf)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	else  // Imprimimos el reporte
	{
		...
		if($lb_valido)	
		{
			...
			/*if ($li_z==1)
			   {
			     $io_encabeza_algo=$io_pdf->openObject();				 
		         //$io_pdf->saveState();
				 $io_pdf->addText(250,45,12,"CONTABILIDAD"); 
				// $io_pdf->restoreState();
				 $io_pdf->closeObject();
				 $io_pdf->addObject($io_encabeza_algo,'all');
				 
			   }
			elseif($li_z==2)
			   {
				 $io_encabeza2=$io_pdf->openObject();
				 //$io_pdf->saveState();				 
			   	 $io_pdf->addText(250,45,12,"CONTABILIDAD 2"); 
				 //$io_pdf->restoreState();
				 $io_pdf->closeObject();
				 $io_pdf->addObject($io_encabeza2,'all');
			   }
			elseif($li_z==3)
			   {
				 $io_pdf->addText(250,45,12,"DPTO. DE BIENES MUEBLES"); 
			   }
			elseif($li_z==4)
			   {
			     $io_pdf->addText(250,45,12,"PROVEEDOR"); 
			...
			if ($row=$io_sql->fetch_row($rs_data))
			{
				...
				uf_print_encabezado_pagina($ls_estcondat,$ls_numordcom,$ld_fecordcom,$ls_coduniadm,$ls_denuniadm,
										   ...
										   $ls_diaplacom,$ls_dirpro,$ls_forpagcom,$ls_nomusuario,$ls_fechentdesde,$ls_fechenthasta,
										   &$io_pdf);
			   ...
			   if ($lb_valido)
			   {
				 ...
				 if ($li_totrows>0)
				 {
				    ...
				    /*for ($i=$li_i+1;$i<=40;$i++)
					    {
						  $la_data2[$i]=array('codigo'=>$ls_codartser,'denominacion'=>$ls_denartser,'cantidad'=>$li_cantartser,
											     'unidad'=>$ls_unidad,'cosuni'=>$ld_preartser,'baseimp'=>$ld_subtotartser,
											     'cargo'=>$ld_carartser,'montot'=>$ld_totartser);
					    }*/
			  ...
			  if(!empty($la_clausulas)&&($ls_estcondat=="S"))
			  {
