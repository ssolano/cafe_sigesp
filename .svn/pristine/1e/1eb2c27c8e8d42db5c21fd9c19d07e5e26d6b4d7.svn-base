		...
		{			
			...
			while($row=$this->io_sql_origen->fetch_row($io_recordset))
			{
				...
				if (!empty($ls_codemp))
				{
					...
					$li_m11= 1;
					$li_m12= 1;
					/* COMENTANDO POR ANAIS SARABIA 23/09/2010
					...
					$ls_periodo= $this->io_validacion->uf_valida_fecha($row["periodo"],'1900-01-01');
                                        $ls_año=substr($row["periodo"],0,4)+1;
                                        $ls_periodo=$ls_año."-".substr($ls_periodo,5);
					...
					$ls_sql="INSERT INTO sigesp_empresa (codemp,nombre,titulo,sigemp,direccion,telemp,faxemp,email,website,m01,".
							...
							"		  '".$li_loncodestpro1."','".$li_loncodestpro2."','".$li_loncodestpro3."','".$li_loncodestpro4."',".
							"		  '".$li_loncodestpro5."','".$li_candeccon."','".$ls_tipconmon."','".$li_redconmon."','".$ls_conrecdoc."')";
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql = "SELECT codemp, nomgru, codsis, nomven,codintper, visible, enabled, leer, incluir, cambiar, eliminar, imprimir, administrativo, anular, ejecutar
				    ...
				    if (!empty($ls_codsis))
				       {
						 ...
						 $ls_sql = "INSERT INTO sss_derechos_grupos (codemp, nomgru, codsis, nomven,codintper, visible, enabled, leer, incluir, cambiar, eliminar, imprimir, administrativo, anular, ejecutar)
						                 VALUES ('".$ls_codemp."','".$ls_nomgru."','".$ls_codsis."','".$ls_nomven."','".$ls_codintper."',".$li_visible.",".$li_enabled.",".$li_leer.",".$li_incluir.",".$li_cambiar.",".$li_eliminar.",".$li_imprimir.",".$li_administrativo.",".$li_anular.",".$li_ejecutar.")";
					     $li_row = $this->io_sql_destino->execute($ls_sql);
					     if ($li_row===false)
					        {
							  $lb_valido=false;
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$ls_sql = "SELECT codmon, denmon, codpai, estmonpri,abrmon
					 FROM sigesp_moneda";
		$io_recordset=$this->io_sql_origen->select($ls_sql);
		if ($io_recordset===false)
		   {  
				    ...
				    if (!empty($ls_codmon))
				       {
						 ...
						 $ld_estmonpri = $this->io_validacion->uf_valida_monto($row["estmonpri"],0);
                                                 $ls_abrmon    = $this->io_validacion->uf_valida_texto($row["abrmon"],0,4,"-");
						 
					     
						 $ls_sql = "INSERT INTO sigesp_moneda (codmon, denmon, codpai, estmonpri, abrmon)
						                               VALUES ('".$ls_codmon."','".$ls_denmon."','".$ls_codpai."','".$ld_estmonpri."','".$ls_abrmon."')";
					     $li_row = $this->io_sql_destino->execute($ls_sql);
					     if ($li_row===false)
					        {
							  $lb_valido=false;
