...
//---------------------------------------------------------------------------------------------------------------------------------------
	...
	else // Imprimimos el reporte
	{
		...
		$fecha_actual_real=$io_fecha->uf_convert_date_to_db(date("d/m/Y"));
		$ls_data="";	
		$li_i=0;
		while ((!$io_report->rs_data->EOF)&&($lb_valido))	
		{
			...
			$ls_anoserinst=calcular_anos(strtotime($ls_fechaIng),strtotime($fecha_actual_real));
			$ls_anosersumtotal=$ls_anoserinst+$li_anoperobr;
			$ls_anoseradmpub=calcular_anos(strtotime($ls_fechaadminpub),strtotime($fecha_actual_real));
			if ($ls_anoserfijo!=0)
			{
			...
			if ($ls_confjub=="1")
			{
				if ($li_codtippersss!='0000010' && $li_codtippersss!='0000014')
				{
					...
					else
					{
					  if ($ls_edad>=$ls_edadm)
					  {
							$ls_difanos=$ls_anot+$li_anoperobr;
							if ($ls_anot>=$ls_anomim)	
							{   
								if (($ls_fechajub=='1900-01-01')&&($valido1)&&($valido2))
								{   
							...
							if (($ls_difanos>$ls_anomim)&&($li_anoperobr>0)&&($ls_anot<$ls_anomim))	
							{  
								if (($ls_fechajub=='1900-01-01'))
								{   
									$li_i++;
									$ls_ano5=substr($ls_fechaadminpub,0,4);
									$ls_ano4=$ls_ano5+$ls_anot;
									$ls_fechaaprox=$ls_ano4.substr($ls_fechaadminpub,4,10);
									$ls_data[$li_i]=array('nro'=>$li_i,'codigo'=>$ls_codper,'nombre'=>$ls_apellido.", ".$ls_nomber,
														 'edad'=>$ls_edad,
														 'fechanac'=>$io_funciones->uf_convertirfecmostrar($ls_fechanac),
							...
							}  // fin si años obreros es mayor de 0
					   }//fin del if de la edad
					  elseif (($ls_edad<=$ls_edadm)&&($ls_anosersumtotal>=35))
					  {
							$ls_difanos=$ls_anot+$li_anoperobr;
							if ($ls_anot>=$ls_anomim)	
							{  
