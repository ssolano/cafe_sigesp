				    ...
				    if (!empty($ls_codemp))
				       {
						 ...
						 $ls_sql = "INSERT INTO scv_transportes (codemp, codtra, codtiptra, dentra, tartra) 
						                                 VALUES ('".$ls_codemp."','".$ls_codtra."','".$ls_codtiptra."','".$ls_dentra."',".$ld_tartra.")";
					     $li_row = $this->io_sql_destino->execute($ls_sql);
					     if ($li_row===false)
					        {
							  $lb_valido=false;
				    ...
				    if (!empty($ls_codemp))
				       {
						 ...
						 $ls_sql = "INSERT INTO scv_tarifakms (codemp, codtar, dentar, kmsdes, kmshas, montar) 
						                           VALUES ('".$ls_codemp."','".$ls_codtar."','".$ls_dentar."',".$ld_kmsdes.",".$ld_kmshas.",".$ld_montar.")";
					     $li_row = $this->io_sql_destino->execute($ls_sql);
					     if ($li_row===false)
					        {
							  $lb_valido=false;
				    ...
				    if (!empty($ls_codemp))
				       {
						 ...
						 $ls_sql = "INSERT INTO scv_otrasasignaciones (codemp, codotrasi, denotrasi, tarotrasi) 
						                           VALUES ('".$ls_codemp."','".$ls_codotrasi."','".$ls_denotrasi."',".$ld_tarotrasi.")";
					     $li_row = $this->io_sql_destino->execute($ls_sql);
					     if ($li_row===false)
					        {
							  $lb_valido=false;
