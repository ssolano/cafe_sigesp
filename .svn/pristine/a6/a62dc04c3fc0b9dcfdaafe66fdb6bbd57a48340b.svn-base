		...
		{   
			...
			while($row=$this->io_sql_origen->fetch_row($io_recordset))
			{
				...
				$ls_expcau = $this->io_validacion->uf_valida_texto($row["expcau"],0,254,"");
                                $li_estcat=$this->io_validacion->uf_valida_monto($row["estcat"],0);
				if($ls_codcau!="")
				{
					$ls_sql="INSERT INTO saf_causas(codcau, dencau, tipcau, estafecon, estafepre, expcau,estcat)".
							...
							"             ".$li_estafepre.",'".$ls_expcau."',".$li_estcat.")";
					$li_row=$this->io_sql_destino->execute($ls_sql);
					if($li_row===false)
					{
						$lb_valido=false;
	...
	function uf_insert_conservacionbien()
	{
		...
		{   
			...
			while($row=$this->io_sql_origen->fetch_row($io_recordset))
			{
				...
				{
					$ls_cadena="Hay data inconsistente en Conservación Bien.\r\n";
					if ($this->lo_archivo)			
					{
						@fwrite($this->lo_archivo,$ls_cadena);
					}
	...
	function uf_insert_activo()
	{
		...
		{   
			...
			while($row=$this->io_sql_origen->fetch_row($io_recordset))
			{
				...
				$ls_codrot= $this->io_validacion->uf_valida_texto($row["codrot"],0,2,"--");
				$ls_codconbie= $this->io_validacion->uf_valida_texto($row["codconbie"],0,2,"02");
				...
				$ls_codmetdep= $this->io_validacion->uf_valida_texto($row["codmetdep"],0,3,"---");
				$ls_catalogo= $this->io_validacion->uf_valida_texto($row["catalogo"],0,15,"---------------");
                                $li_costo= $this->io_validacion->uf_valida_monto($row["costo"],0);
				$li_cossal= $this->io_validacion->uf_valida_monto($row["cossal"],0);
				$li_monordcom= $this->io_validacion->uf_valida_monto($row["monordcom"],0);
				$li_moncobase= $this->io_validacion->uf_valida_monto($row["moncobase"],0);
				...
				$ls_obsact= $this->io_validacion->uf_valida_texto($row["obsact"],0,800,"");
	...
	function uf_insert_depreciacion()
	{
		...
		{   
			...
			while($row=$this->io_sql_origen->fetch_row($io_recordset))
			{
				...
				$li_mondepmen= $this->io_validacion->uf_valida_monto($row["mondepmen"],0);
				$li_mondepano= $this->io_validacion->uf_valida_monto($row["mondepano"],0);
				$li_mondepacu= $this->io_validacion->uf_valida_monto($row["mondepacu"],0);
				$ls_estcon= $this->io_validacion->uf_valida_monto($row["estcon"],0);
				...
				if($ls_codact!="")
				{
					$ls_sql="INSERT INTO saf_depreciacion(codemp, codact, ideact, fecdep, mondepmen, mondepano, mondepacu,".
							...
							"             ".$li_mondepano.",".$li_mondepacu.",".$ls_estcon.",'".$ld_fechaconta."','".$ld_fechaanula."')";
					$li_row=$this->io_sql_destino->execute($ls_sql);
					if($li_row===false)
					{
						$lb_valido=false;
