...
<input name="txtmonto" type="text" id="txtmonto" style="text-align:right" onBlur="javascript:uf_format(this);uf_montoobjret(this);uf_verificar_monto(this);" onKeyPress="return(currencyFormat(this,'.',',',event));" value="<?php print number_format($ldec_montomov,2,",",".");?>" size="24" maxlength="22">	  </div></td>
					 ...
					 if (lb_valido)
						{
						  ...
						  ls_pagina = "sigesp_w_regdt_contable.php?txtprocedencia=SCBMOV&mov_document="+ls_documento+"&fecha="+ld_fecha+"&procede="+ls_procede+"&tipo="+ls_tipo+
									  ...
									  "&anticipo="+ls_anticipo+"&codtipfon="+ls_codtipfon+"&numordpagmin="+ls_numordpagmin;
						  ...
						  if (lb_valido)
							 {
							   ...
							   ls_pagina = "sigesp_w_regdt_presupuesto.php?txtprocedencia=SCBMOV&mov_document="+ls_documento+"&fecha="+ld_fecha+"&procede="+ls_procede+
										  ...
										  "&estmov="+ls_estmov+"&codconmov="+ls_codconmov+"&tip_mov= &opener=sigesp_scb_p_movbanco.php&estdoc="+ls_estdoc+"&codfuefin="+ls_codfuefin+"&numordpagmin="+ls_numordpagmin+"&codtipfon="+ls_codtipfon;
						  ...
						  if (lb_valido)
							 {
								...
								ls_pagina = "sigesp_w_regdt_deducciones.php?txtdocumento="+ls_documento+"&txtprocede=SCBMOV&mov_document="+ls_documento+"&fecha="+ld_fecha+
											...
											"&tip_mov= &opener=sigesp_scb_p_movbanco.php&estdoc="+ls_estdoc+"&codfuefin="+ls_codfuefin+"&codtipfon="+ls_codtipfon+"&numordpagmin="+ls_numordpagmin;
...
function uf_verificar_retenido()
{
	...
	ldec_monret= ldec_monobjret - ldec_monto; 			 // Cambio agregado
	ldec_monret=uf_convertir_monto(ldec_monret);		 // Cambio agregado
	ldec_monret    = parseFloat(ldec_monret);		  	 // Cambio agregado
	if(ldec_monto<ldec_monobjret)
	{
		if(ldec_monret>0)
		{
			...
			alert("Monto de retenci�n calculado: "+ldec_monret);
		}			
	}
	else
	{
		if(ldec_monto>ldec_monobjret)
		{
			alert("Monto Objeto a Retenci�n no puede ser menor al monto del movimiento !!!");
			f.txtmonobjret.value=uf_convertir(ldec_monto);
			ldec_monret= 0;							 			 // Cambio agregado
			ldec_monret=uf_convertir_monto(ldec_monret);		 // Cambio agregado
			ldec_monret    = parseFloat(ldec_monret);		  	 // Cambio agregado
			f.txtretenido.value=uf_convertir(ldec_monret);		 // Cambio agregado
		}
		
		if(ldec_monto==ldec_monobjret)
		{
			alert("No se est�n realizado Retenci�nes para el presente movimiento!!!");
