...
$io_seguridad= new sigesp_c_seguridad();
	...
	$ls_sistema="CFG";
	$ls_ventanas="sigesp_scb_d_tipocolocacion.php";
	$la_security[1]=$ls_empresa;
	$la_security[2]=$ls_sistema;
	$la_security[3]=$ls_logusr;
	$la_security[4]=$ls_ventanas;
	$in_classtipocolocacion=new sigesp_scb_c_tipocolocacion($la_security);
...
if ($ls_operacion == "ELIMINAR")
   {
     $lb_existe = $in_classtipocolocacion->uf_select_tipocolocacion($ls_codigo);
	 if ($lb_existe)
	    {
...
function ue_nuevo()
{
	...
	if (li_incluir==1)
	   {	
         ...
         f.action="sigesp_scb_d_tipocolocacion.php";
		 f.submit();
	   }
	else
	   {
...
function ue_buscar()
{
	...
	if (li_leer==1)
       {
	     window.open("sigesp_scb_cat_tipocolocacion.php","catalogo","menubar=no,toolbar=no,scrollbars=yes,width=550,height=400,left=50,top=50,location=no,resizable=yes");
       }
	else
	   {
		 alert("No tiene permiso para realizar esta operación");
