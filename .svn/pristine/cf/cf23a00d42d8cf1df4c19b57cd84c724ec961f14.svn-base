<?php
	...
	if(rtrim($la_data["archrtf"][1])!="")
	{
		...
		header('Location: sigesp_scb_rpp_cartaorden_word.php?codigo='.$ls_codigo.'&codban='.$ls_codban.'&ctaban='.$ls_ctaban.'&numdoc='.$ls_numdoc.'&codope=ND'.'&tipproben='.$ls_tipproben);
	}
	...
	if(!array_key_exists("opener",$_GET))
	{
		...
		if(($lb_valido) || (count ($la_cartaorden)==0)) // le quitamos la negacion a la variable lb_valido
		{
