	...
	if ($ls_opener!="")
	{
		if($ls_opener=="anticipo" || $ls_opener=="variacion")
		{
			$ls_sql="SELECT c.codcon, c.monto, c.placon,c.placonuni, c.mulcon, c.fecfincon, c.tiemulcon, c.mulreuni,c.lapgarcon, c.lapgaruni,  c.codtco, a.cod_pro, a.cod_pro_ins,
					c.monmaxcon, c.pormaxcon,  c.obscon,  c.porejefiscon, t.nomtco, c.porejefincon, c.monejefincon,c.codasi,o.codobr,o.desobr,c.feccon,c.fecinicon,c.estcon,c.precon,c.estspgscg
	...
	}else
	{
		...
		if($row=$io_sql->fetch_row($rs_data))
		{
			...
			for($li_z=1;$li_z<=$li_totrow;$li_z++)
			{
				...
				'$ls_obscon','$ls_porejefiscon','$ls_porejefincon','$ls_monejefincon','$ls_codasi','$ls_feccrecon','$ls_fecinicon','$ls_nomtco','$ls_codobr','$ls_codpro','$ls_codproins','$ls_fecfincon','$ls_precon');\">".$ls_precon.$ls_codigo."</a></td>";
