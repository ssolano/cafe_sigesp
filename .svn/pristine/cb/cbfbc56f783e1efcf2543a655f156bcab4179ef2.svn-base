...
class sigesp_copia_sno {
	...
	function ue_copiar_nomina_basico()
	{
		...
		{	
			$lb_valido=$this->uf_insert_rango();
                ...
                if($lb_valido)
		{
			$lb_valido=$this->uf_insert_tipopersonalsss();
		}
                if($lb_valido)
		{
			$lb_valido=$this->uf_insert_organigrama();
		}
	...
	function uf_insert_unidadadministrativa()
	{
		...
		$li_total_select=0;
		$li_total_insert=0;
		$ls_sql="SELECT codemp, minorguniadm, ofiuniadm, uniuniadm, depuniadm, prouniadm, desuniadm, codprouniadm, codproviauniadm, estcla ".
		...
		else
		{
			...
			while($row=$this->io_sql_origen->fetch_row($io_recordset))
			{
				...
				$ls_codproviauniadm = $this->io_validacion->uf_valida_texto($row["codproviauniadm"],0,33,"");
                                $ls_estcla		= $this->io_validacion->uf_valida_texto($row["estcla"],0,1,"00");
				...
				if(($ls_minorguniadm!="")&&($ls_ofiuniadm!="")&&($ls_uniuniadm!="")&&($ls_depuniadm!="")&&($ls_prouniadm!=""))
				{
					$ls_sql="INSERT INTO sno_unidadadmin (codemp,minorguniadm,ofiuniadm,uniuniadm,depuniadm,prouniadm,desuniadm,".
							...
							"'".$ls_prouniadm."','".$ls_desuniadm."','".$ls_codprouniadm."','".$ls_codproviauniadm."','".$ls_estcla."')";
	...
	function uf_insert_tipopersonalsss()
	{
		...
		// Fecha Creación: 13/09/2010 								Fecha Última Modificación :
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$li_total_select=0;
		$li_total_insert=0;
		...
		{
			$lb_valido=false;
			$ls_cadena="Error al Seleccionar las Tablas de Tipo de Personal SSS.\r\n".$this->io_sql_origen->message."\r\n";
			$ls_cadena=$ls_cadena.$ls_sql."\r\n";
			if ($this->lo_archivo)
			{
				@fwrite($this->lo_archivo,$ls_cadena);
		...
		else
		{
			...
			{
				$ls_codemp    = $row["codemp"];
				$ls_codtippersss = $this->io_validacion->uf_valida_texto($row["codtippersss"],0,7,"");
				$ls_dentippersss = $this->io_validacion->uf_valida_texto($row["dentippersss"],0,254,"");
				if($ls_codtippersss!="")
				{
					$ls_sql="INSERT INTO sno_tipopersonalsss(codemp,codtippersss,dentippersss)".
							"VALUES('".$ls_codemp."','".$ls_codtippersss."','".$ls_dentippersss."')";
					$li_row=$this->io_sql_destino->execute($ls_sql);
					if($li_row===false)
					{
						$lb_valido=false;
						$ls_cadena="Error al Insertar las Tablas de Tipo de Personal SSS.\r\n".$this->io_sql_destino->message."\r\n";
						$ls_cadena=$ls_cadena.$ls_sql."\r\n";
						if ($this->lo_archivo)
						{
							@fwrite($this->lo_archivo,$ls_cadena);
				...
				{
					$ls_cadena="Hay data inconsistente en Tipo de Personal SSS.\r\n";
			...
			$ls_cadena="//*****************************************************************//\r\n";
			$ls_cadena=$ls_cadena."   Tabla Origen  sno_tipopersonalsss Registros ".$li_total_select." \r\n";
			$ls_cadena=$ls_cadena."   Tabla Destino sno_tipopersonalsss Registros ".$li_total_insert." \r\n";
			$ls_cadena=$ls_cadena."//*****************************************************************//\r\n";
			if ($this->lo_archivo)
			{
				@fwrite($this->lo_archivo,$ls_cadena);
	...
	function uf_insert_organigrama()
	{
		...
		{
			$lb_valido=false;
			$ls_cadena="Error al Seleccionar las Tablas de Organigrama.\r\n".$this->io_sql_origen->message."\r\n";
			$ls_cadena=$ls_cadena.$ls_sql."\r\n";
			if ($this->lo_archivo)
			{
				@fwrite($this->lo_archivo,$ls_cadena);
		...
		else
		{
			...
			{
				$ls_codemp    = $row["codemp"];
				$ls_codorg = $this->io_validacion->uf_valida_texto($row["codorg"],0,10,"----------");
                                $ls_desorg = $this->io_validacion->uf_valida_texto($row["desorg"],0,254,"");
				$ls_padorg = $this->io_validacion->uf_valida_texto($row["padorg"],0,10,"----------");
                                $ls_nivorg=$this->io_validacion->uf_valida_monto($row["nivorg"],2);
				if($ls_codemp!="")
				{
					$ls_sql="INSERT INTO srh_organigrama(codemp,codorg,desorg,nivorg,padorg)".
							"VALUES('".$ls_codemp."','".$ls_codorg."','".$ls_desorg."','".$ls_nivorg."','".$ls_padorg."')";
					$li_row=$this->io_sql_destino->execute($ls_sql);
                                        
					if($li_row===false)
					{
						$lb_valido=false;
						$ls_cadena="Error al Insertar las Tablas Organigrama.\r\n".$this->io_sql_destino->message."\r\n";
						$ls_cadena=$ls_cadena.$ls_sql."\r\n";
						if ($this->lo_archivo)
						{
							@fwrite($this->lo_archivo,$ls_cadena);
				...
				{
					$ls_cadena="Hay data inconsistente en Organigrama.\r\n";
			...
			$ls_cadena="//*****************************************************************//\r\n";
			$ls_cadena=$ls_cadena."   Tabla Origen  srh_organigrama Registros ".$li_total_select." \r\n";
			$ls_cadena=$ls_cadena."   Tabla Destino srh_organigrama Registros ".$li_total_insert." \r\n";
			$ls_cadena=$ls_cadena."//*****************************************************************//\r\n";
			if ($this->lo_archivo)
			{
				@fwrite($this->lo_archivo,$ls_cadena);
	...
	function uf_insert_personal()
	{
		...
		$ls_sql="SELECT codemp, codper, cedper, nomper, apeper, dirper, fecnacper, edocivper, telhabper, telmovper, sexper, ".
				...
				"		codcom, codran, numexpper, codpainac, codestnac, codorg ".
				"  FROM sno_personal ";
		$io_recordset=$this->io_sql_origen->select($ls_sql);
		if($io_recordset===false)
		{ 
			...
			{  
				...
				if(($ls_codper!="")&&($ls_cedper!="")&&($ld_fecingadmpubper!="")&&($ld_fecingper!=""))
				{
					$ls_sql="INSERT INTO sno_personal(codemp,codper,cedper,nomper,apeper,dirper,fecnacper,edocivper,telhabper,telmovper,".
							...
							"			 codran, numexpper, codpainac, codestnac,codorg)".
							...
							"			  '".$ls_codcom."','".$ls_codran."','".$ls_numexpper."','".$ls_codpainac."','".$ls_codestnac."','".$ls_codorg."')";
					$li_row=$this->io_sql_destino->execute($ls_sql);
					if($li_row===false)
					{
						$lb_valido=false;
			...
			while(($row=$this->io_sql_origen->fetch_row($io_recordset))&&$lb_valido)
			{
				...
				if(($ls_codper!="")&&($li_codtraant!=""))
				{
					$ls_sql="INSERT INTO sno_trabajoanterior(codemp, codper, codtraant, emptraant, ultcartraant, ultsuetraant, fecingtraant, ".
							"fecrettraant, emppubtraant,codded, anolab, meslab, dialab) VALUES ('".$ls_codemp."','".$ls_codper."',".
	...
	function uf_insert_asignacioncargo($as_codemp,$as_codnom,$as_codnomnuevo)
	{
		...
		$ls_sql="SELECT codemp, codnom, codasicar, denasicar, claasicar, minorguniadm, ofiuniadm, uniuniadm, depuniadm, ".
				"		prouniadm, codtab, codpas, codgra, codded, codtipper, numvacasicar, numocuasicar, codproasicar, estcla ".
			   ...
			   while(($row=$this->io_sql_origen->fetch_row($io_recordset))&&$lb_valido)
			   {
				...
				if(($ls_codnom!="")&&($ls_codper!=""))
				{
					$ls_sql="INSERT INTO sno_personalnomina(codemp,codnom,codper,codsubnom,codasicar,codtab,codgra,codpas,sueper,horper,".
							...
							"			 '".$ls_codclavia."','".$ls_codunirac."',".$li_pagtaqper.")";
					$li_row=$this->io_sql_destino->execute($ls_sql);
					if($li_row===false)
					{
						$lb_valido=false;
	...
	function uf_insert_constante($as_codemp,$as_codnom,$as_codnomnuevo)
	{
		...
		else
		{
			   ...
			   while(($row=$this->io_sql_origen->fetch_row($io_recordset))&&$lb_valido)
			   {
				...
				{
					$ls_sql="INSERT INTO sno_constante(codemp,codnom,codcons,nomcon,unicon,equcon,topcon,valcon,reicon,tipnumcon, conespseg)".
			...
			while(($row=$this->io_sql_origen->fetch_row($io_recordset))&&$lb_valido)
			{
				...
				if(($ls_codnom!="")&&($ls_codper!="")&&($ls_codtippre!=""))
				{
					$ls_sql="INSERT INTO sno_prestamos(codemp,codnom,codper,numpre,codtippre,codconc,monpre,numcuopre,perinipre,".
							...
							"			  ".$li_stapre.",'".$ld_fecpre."','".$ls_obsrecpre."','".$ls_obssuspre."')";
					$li_row=$this->io_sql_destino->execute($ls_sql);
					if($li_row===false)
					{
						$lb_valido=false;
	...
	function uf_insert_periodos($as_codemp,$as_codnom,$as_codnomnuevo)  
	{
		...
		$ld_periodo="";
		$ls_sql="SELECT date_part('year',periodo) as periodo ".
				"  FROM sigesp_empresa ".
				" WHERE codemp='".$as_codemp."'";
		...
		{
			$lb_valido = false;
			$this->io_mensajes->message("CLASE->sigesp_copia_sno METODO->uf_insert_periodos ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql_destino->message));
		}
		else
		{
