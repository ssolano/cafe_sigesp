...
function sigesp_scb_c_banco($aa_security)
{
	...
	function uf_guardar_banco(&$ls_codigo,$ls_nombre,$ls_direccion,$ls_gerente,$ls_telefono,$ls_celular,$ls_email,$li_esttesnac,$ls_status,$ls_codsude,$ls_swift)
	{
		$ls_codemp= $this->datemp["codemp"];
		...
		if($ls_status!='C')//Si no existe lo inserto
		{
			...
			$ls_cadena= " INSERT INTO scb_banco(codemp,codban,nomban,dirban,gerban,telban,conban,movcon,esttesnac,codsudeban,swift) ".
			" VALUES('".$ls_codemp."','".$ls_codigo."','".$ls_nombre."','".$ls_direccion."','".$ls_gerente."','".$ls_telefono."','".$ls_email."','".$ls_celular."',".$li_esttesnac.",'".$ls_codsude."','".$ls_swift."') ";
		...
		else
		{
			if($ls_status=="C")
			{
				$ls_cadena= " UPDATE scb_banco SET nomban='".$ls_nombre."',gerban='".$ls_gerente."',dirban='".$ls_direccion."',telban='".$ls_telefono."',conban='".$ls_email."',movcon='".$ls_celular."',esttesnac=".$li_esttesnac.",codsudeban='".$ls_codsude."',swift='".$ls_swift."' WHERE codemp='".$ls_codemp."' AND codban='".$ls_codigo."'";
