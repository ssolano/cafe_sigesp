...
function uf_obtener_mov_conciliacion($ls_mesano,$ls_codban,$ls_ctaban,$ldec_salseglib,$ldec_salsegbco)
	...
	----------------------------------------------------------------------------*/
		...
		$ls_sql="SELECT numdoc
			     ...
			     WHERE codemp='".$ls_codemp."' 
				 AND numdoc='$as_numdoc' 
				 AND codban='$as_codban' AND ctaban='$as_ctaban' 
				 AND codope='ND'"; // se agrego comilla para que funcionara el select	
	...
	----------------------------------------------------------------------------*/
		...
		{
			$ls_sql = "  SELECT max(scb_movbco.ctaban) as ctaban,scb_movbco.$ls_campo, ".
