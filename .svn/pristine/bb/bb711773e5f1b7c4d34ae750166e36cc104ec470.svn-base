       ...
       if ($row=$io_sql->fetch_row($rs_data))
          {
	        ...
	        $rs_datos["ctabene"] = trim($row["codcueban"]);
				 ...
				 if (!empty($ds_banco))
				    {
					  ...
					  if ($li_numrows>0)
						 { 
						   ...
						   @mkdir($ls_ruta,0755);
						   $lb_valido = $io_metodobanco->uf_metodo_banco($ls_ruta,$ls_nommetban,'','','',$ld_fecha,$ldec_montomov,$ls_ctaban,$ds_banco_nomina,$ls_metban,$ls_consol,$la_seguridad);
						 } 
					}
			   }
