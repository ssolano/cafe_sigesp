...
$lo_dao = new sps_pro_deudaanterior_dao();
...
if ($ls_operacion=="ue_inicializar")
{
    ...
    $lb_hay = $lo_dao->getDeudaAnterior("ORDER BY feccordeuant ASC", $la_datos);
...
else
{
  ...
  $lb_hay = $lo_dao->getDeudaAnterior("ORDER BY $sort_col $sort_dir LIMIT $offset,$pagesize",$la_datos);
  if ($lb_hay)
  {
    $li_registros = (count($la_datos,COUNT_RECURSIVE)/count($la_datos))-1;  
    ...
    for ($i=1; $i<=$li_registros; $i++)
    {
	  ...
	  $la_arr[$li_i++] = $ls_feccordeuant = $io_function->uf_convertirfecmostrar($la_datos["feccordeuant"][$i]);
	  $la_arr[$li_i++] = $ls_deuantant = str_replace(".", ",", $la_datos["deuantant"][$i]);
	  $la_arr[$li_i++] = $ls_deuantint = str_replace(".", ",", $la_datos["deuantint"][$i]);
	  $la_arr[$li_i++] = $ls_antpag    = str_replace(".", ",", $la_datos["antpag"][$i]);
      ...
      $ls_salida .='<tr class="celdas-blancas">';
	  $ls_salida .='<td align="center" width="200"><a href="javascript:ue_seleccionar('.$la_newarr.');">'.$ls_feccordeuant.'</a></td>';
	  $ls_salida .='</tr>';
    }
