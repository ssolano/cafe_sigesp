...
<?php	
	...
	require_once("sigesp_sss_c_derechogrupo.php");
	$io_sss= new sigesp_sss_c_derechogrupo();
			...
			if($lb_existe)
			{
				if(array_key_exists("radioenable",$_POST))
				{
					...
					if(($ls_codsis=="SNO")||($ls_codsis=="SPG"))
					{
						...
						if ($lb_valido)
						{
						 $lb_valido=$io_sss->uf_sss_load_usuariosgrupo($ls_codemp,$ls_nomgru,&$la_usuarios);
						 if ($lb_valido)
						 {
							$li_totalusuarios=count($la_usuarios);
							for($li_s=1;$li_s<=$li_totalusuarios;$li_s++)
							{
								$ls_codusu=$la_usuarios[$li_s];
						...
						{
							if($ls_codsis=='SNO')
							{$io_msg->message("Los usuarios no tienen ninguna Nomina asociada");}
							if($ls_codsis=='SPG')
							{$io_msg->message("Los usuarios no tienen ninguna Estructura Presupuestaria asociada");}
							$lb_valido=false;
						}
					}
					else
