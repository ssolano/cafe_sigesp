...
//////////////////////////////////////////////////////////////////////////////
  ...
  $ls_tipoctaban        = $ar_datos["tipoctabanco"];
  ...
  if (!$this->uf_select_validar_rif($as_codemp,$ls_rifpro,$ls_codigo))
  ...
  else
	 {
	  $ls_sql=" INSERT INTO rpc_proveedor(codemp,cod_pro,nompro,dirpro,telpro,faxpro,nacpro,codtipoorg,                      ".
			  ...
			  " tipconpro,sc_cuentarecdoc,sc_ctaant,tipperpro,tipoctaban)".
			  " VALUES                                                                                                       ".
			  ...
			  " '".$ls_graemp."','".$ls_emailrep."','".$ls_codbansig."','".$ls_tipconpro."','".$ls_contablerecdoc."','".$ls_ctaant."','".$ls_tipperpro."',".$ls_tipoctaban.")";  
	  $this->io_sql->begin_transaction(); 
	  $rs_data=$this->io_sql->execute($ls_sql);
	  if ($rs_data===false)
	     { 
		   $lb_valido=false;
		   $this->is_msg_error=$ls_sql."CLASE->SIGESP_RPC_C_PROVEEDOR; METODO->uf_insert_proveedor; ERROR->".$this->io_funcion->uf_convertirmsg($this->io_sql->message);
		 }
	  else
	     { 
		   $lb_valido=true;
...
//////////////////////////////////////////////////////////////////////////////
 ...
 else
 {
  $ls_sql=" UPDATE rpc_proveedor ".
		  ...
		  " tipperpro = '".$ls_tipperpro."', sc_cuentarecdoc = '".$ls_contablerecdoc."', sc_ctaant='".$ls_ctaant."', tipoctaban='".$ls_tipoctaban."' ".
