...
class sigesp_rpc_c_benexded
{
	...
	function uf_load_deduccion($as_cedbene,&$ai_totrows,&$ao_object)
	{
		...
		//		   Access: public 
		//	    Arguments: as_cedbene  // código de Beneficiario
		//				   ai_totrows  // total de fila
		//				   ao_object  // arreglo de objetos
		//	      Returns: lb_valido True si se encontro ó False si no se encontró
		//	  Description: Funcion que obtiene todas las deducciones según el tipo de Beneficiario
		//	   Creado Por: Ing. Yesenia Moreno
		// Fecha Creación: 27/03/2009 								Fecha Última Modificación : 
		/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Beneficiario-Deducciones MÉTODO->uf_load_deduccion ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
			$ai_totrows=0;
    ...
    function uf_eliminar_deduccion($as_cedbene,$aa_seguridad)
    {
		...
		{
			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Beneficiario-Deducciones MÉTODO->uf_eliminar_deduccion ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="DELETE";
			$ls_descripcion ="Eliminó las Deducciones asociadas al Beneficiario ".$as_cedbene;
	...
	function uf_insert_deducciones($as_cedbene,$as_codded,$aa_seguridad)
	{
		...
		{
 			$lb_valido=false;
			$this->io_mensajes->message("CLASE->Beneficiario-Deducciones MÉTODO->uf_insert_deducciones ERROR->".$this->io_funciones->uf_convertirmsg($this->io_sql->message)); 
		}
		else
		{
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
			$ls_evento="INSERT";
			$ls_descripcion ="Insertó La deducción ".$as_codded." al Beneficiario ".$as_cedbene;
			$lb_valido= $this->io_seguridad->uf_sss_insert_eventos_ventana($aa_seguridad["empresa"],
											$aa_seguridad["sistema"],$ls_evento,$aa_seguridad["logusr"],
											$aa_seguridad["ventanas"],$ls_descripcion);
			/////////////////////////////////         SEGURIDAD               /////////////////////////////		
