...
$ls_sql=" SELECT coddoc,dendoc,tipdoc ".
        " FROM rpc_documentos ".
		" ORDER BY coddoc ASC";		
...
if($row=$io_sql->fetch_row($rs_documento))
{
	...
	for($z=1;$z<=$totrow;$z++)
	{
		...
		print "<td><a href=\"javascript: aceptar('$codigo','$denominacion','$tipo');\">".$codigo."</a></td>";
...
<script language="JavaScript">
  function aceptar(codigo,denominacion,tipo)
  {
	...
	opener.document.form1.cmbtipdoc[0].selected=tipo;
	
	opener.document.form1.cmbtipdoc.value=tipo;
