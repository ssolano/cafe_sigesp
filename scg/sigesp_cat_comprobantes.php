...
if($ls_operacion=="BUSCAR")
{
		...
		if($_SESSION["ls_gestor"]=='MYSQLT')
		{
			$ls_sql="SELECT   ( CASE   cmp.tipo_destino
								...
								ELSE 'Ninguno'
								END )  as  nombre,cmp.codemp,cmp.procede,cmp.comprobante,
								cmp.descripcion,cmp.fecha,cmp.cod_pro,cmp.ced_bene,cmp.tipo_destino,cmp.codban, cmp.ctaban,
		...
		if($_SESSION["ls_gestor"]=='POSTGRES')
		{
			$ls_sql="SELECT ( CASE WHEN cmp.tipo_destino='P' THEN prv.nompro 
							  ...
							  ELSE 'Ninguno' END ) as nombre,cmp.codemp,cmp.procede,
							  cmp.comprobante,cmp.descripcion,cmp.fecha,cmp.cod_pro,cmp.codban, cmp.ctaban,
		...
		if($_SESSION["ls_gestor"]=='INFORMIX')
		{
			...
			$ls_sql="SELECT ( CASE WHEN cmp.tipo_destino='P' THEN prv.nompro 
							  WHEN cmp.tipo_destino='B' THEN  TRIM(xbf.apebene)||','||xbf.nombene
							  ELSE 'Ninguno' END ) as nombre,cmp.codemp,cmp.procede,
							  cmp.comprobante,cmp.descripcion,cmp.fecha,cmp.cod_pro,cmp.codban, cmp.ctaban,
							cmp.ced_bene,cmp.tipo_destino,
								(SELECT SUM(scg_dt_cmp.monto) 
