...
class sigesp_copia_scgspgspi {
			...
			while($row=$this->io_sql_origen->fetch_row($io_recordset))
			{
				...
				if($ls_sc_cuenta!="")
				{
					$ls_sql="INSERT INTO scg_cuentas (codemp, sc_cuenta, denominacion, status, asignado, distribuir, enero, febrero, marzo, abril, ".
							"mayo, junio, julio, agosto, septiembre, octubre, noviembre, diciembre, nivel, referencia) VALUES('".$ls_codemp."','".$ls_sc_cuenta."','".$ls_denominacion."','".$ls_status."',".
	...
	function uf_insert_spgunidadadministrativa()
	{
		...
		{   
			...
			while($row=$this->io_sql_origen->fetch_row($io_recordset))
			{
				...
				if($ls_coduniadm!="")
				{
					$ls_sql="INSERT INTO spg_unidadadministrativa(codemp,coduniadm,coduac,denuniadm,estemireq,".
							" coduniadmsig) VALUES('".$ls_codemp."','".$ls_coduniadm."','".$ls_coduac."',".
	...
	function uf_insert_spgestructura1()
	{
		...
		{   
			...
			{
				$ls_codemp = $row["codemp"]; 
				$ls_codestpro1 = $this->io_validacion->uf_valida_texto($row["codestpro1"],0,25,"");
	...
	function uf_insert_spgestructura2()
	{
		...
		$li_total_select = 0;
		$li_total_insert = 0;
		$ls_sql          = "SELECT codemp, codestpro1, estcla, codestpro2, denestpro2 ".
						   "  FROM spg_ep2 ";
		$io_recordset    = $this->io_sql_origen->select($ls_sql);
		if($io_recordset===false)
		{ 
		...
		{   
			...
			while($row=$this->io_sql_origen->fetch_row($io_recordset))
			{
				$ls_codemp = $row["codemp"]; 
				$ls_codestpro1 = $this->io_validacion->uf_valida_texto($row["codestpro1"],0,25,"");
				$ls_codestpro2 = $this->io_validacion->uf_valida_texto($row["codestpro2"],0,25,"");
				$ls_denestpro2 = $this->io_validacion->uf_valida_texto($row["denestpro2"],0,254,"");
                                $ls_estcla = $this->io_validacion->uf_valida_texto($row["estcla"],0,1,"");
				if($ls_codestpro1!="")
				{
					$ls_sql="INSERT INTO spg_ep2 (codemp, codestpro1,estcla, codestpro2, denestpro2) VALUES('".$ls_codemp."','".$ls_codestpro1."','".$ls_estcla."',".
	...
	function uf_insert_spgestructura3()
	{
		...
		$li_total_select = 0;
		$li_total_insert = 0;
		$ls_sql          = "SELECT codemp, codestpro1,estcla, codestpro2, codestpro3, denestpro3, codfuefin ".
						   "  FROM spg_ep3 ";
		$io_recordset    = $this->io_sql_origen->select($ls_sql);
		if($io_recordset===false)
		{ 
		...
		{   
			...
			while($row=$this->io_sql_origen->fetch_row($io_recordset))
			{
				...
				$ls_codestpro1 = $this->io_validacion->uf_valida_texto($row["codestpro1"],0,25,"");
				$ls_codestpro2 = $this->io_validacion->uf_valida_texto($row["codestpro2"],0,25,"");
				$ls_codestpro3 = $this->io_validacion->uf_valida_texto($row["codestpro3"],0,25,"");
				$ls_denestpro3 = $this->io_validacion->uf_valida_texto($row["denestpro3"],0,254,"");
				$ls_codfuefin = $this->io_validacion->uf_valida_texto($row["codfuefin"],0,2,"");
                                $ls_estcla = $this->io_validacion->uf_valida_texto($row["estcla"],0,1,"");
				if($ls_codestpro1!="")
				{
					$ls_sql="INSERT INTO spg_ep3 (codemp, codestpro1,estcla, codestpro2, codestpro3, denestpro3, codfuefin) VALUES('".$ls_codemp."',".
							"'".$ls_codestpro1."','".$ls_estcla."','".$ls_codestpro2."','".$ls_codestpro3."','".$ls_denestpro3."','".$ls_codfuefin."')";
					$li_row=$this->io_sql_destino->execute($ls_sql);
					if($li_row===false)
					{
						$lb_valido=false;
	...
	function uf_insert_spgestructura4()
	{
		...
		$li_total_select = 0;
		$li_total_insert = 0;
		$ls_sql          = "SELECT codemp, codestpro1, estcla, codestpro2, codestpro3, codestpro4, denestpro4 ".
						   "  FROM spg_ep4 ";
		$io_recordset    = $this->io_sql_origen->select($ls_sql);
		if($io_recordset===false)
		{ 
		...
		{   
			...
			while($row=$this->io_sql_origen->fetch_row($io_recordset))
			{
				...
				$ls_codestpro2 = $this->io_validacion->uf_valida_texto($row["codestpro2"],0,25,"");
				$ls_codestpro3 = $this->io_validacion->uf_valida_texto($row["codestpro3"],0,25,"");
				$ls_codestpro4 = $this->io_validacion->uf_valida_texto($row["codestpro4"],0,25,"");
				$ls_denestpro4 = $this->io_validacion->uf_valida_texto($row["denestpro4"],0,254,"");
                                $ls_estcla = $this->io_validacion->uf_valida_texto($row["estcla"],0,1,"");
				if($ls_codestpro1!="")
				{
					$ls_sql="INSERT INTO spg_ep4 (codemp, codestpro1, estcla, codestpro2, codestpro3, codestpro4, denestpro4) VALUES('".$ls_codemp."',".
							"'".$ls_codestpro1."','".$ls_estcla."','".$ls_codestpro2."','".$ls_codestpro3."','".$ls_codestpro4."','".$ls_denestpro4."')";
					$li_row=$this->io_sql_destino->execute($ls_sql);
					if($li_row===false)
					{
						$lb_valido=false;
	...
	function uf_insert_spgestructura5()
	{
		...
		$li_total_select = 0;
		$li_total_insert = 0;
		$ls_sql          = "SELECT codemp, codestpro1,estcla, codestpro2, codestpro3, codestpro4, codestpro5, denestpro5, codfuefin ".
						   "  FROM spg_ep5 ";
		$io_recordset    = $this->io_sql_origen->select($ls_sql);
		if($io_recordset===false)
		{ 
		...
		{   
			...
			while($row=$this->io_sql_origen->fetch_row($io_recordset))
			{
				...
				$ls_codestpro3 = $this->io_validacion->uf_valida_texto($row["codestpro3"],0,25,"");
				$ls_codestpro4 = $this->io_validacion->uf_valida_texto($row["codestpro4"],0,25,"");
				$ls_codestpro5 = $this->io_validacion->uf_valida_texto($row["codestpro5"],0,25,"");
				$ls_denestpro5 = $this->io_validacion->uf_valida_texto($row["denestpro5"],0,254,"");
				$ls_codfuefin = $this->io_validacion->uf_valida_texto($row["codfuefin"],0,2,"");
                                $ls_estcla = $this->io_validacion->uf_valida_texto($row["estcla"],0,1,"");
				if($ls_codestpro1!="")
				{
					$ls_sql="INSERT INTO spg_ep5 (codemp, codestpro1,estcla, codestpro2, codestpro3, codestpro4, codestpro5, denestpro5, codfuefin) ".
							"VALUES('".$ls_codemp."','".$ls_codestpro1."','".$ls_estcla."','".$ls_codestpro2."','".$ls_codestpro3."','".$ls_codestpro4."',".
	...
	function uf_insert_spgcuentas()
	{
		...
		$li_total_select = 0;
		$li_total_insert = 0;
		$ls_sql          = "SELECT codemp,estcla, codestpro1, codestpro2, codestpro3, codestpro4, codestpro5, spg_cuenta, denominacion, status, sc_cuenta, distribuir, nivel, referencia ".
						   "  FROM spg_cuentas ";
		$io_recordset    = $this->io_sql_origen->select($ls_sql);
		if($io_recordset===false)
		{ 
		...
		{   
			...
			while($row=$this->io_sql_origen->fetch_row($io_recordset))
			{
				...
				if($ls_spgcuenta!="")
				{
					$ls_sql="INSERT INTO spg_cuentas (codemp,estcla, codestpro1, codestpro2, codestpro3, codestpro4, codestpro5, spg_cuenta, denominacion, ".
							...
							" VALUES('".$ls_codemp."','".$ls_estcla."','".$ls_codestpro1."','".$ls_codestpro2."','".$ls_codestpro3."','".$ls_codestpro4."',".
