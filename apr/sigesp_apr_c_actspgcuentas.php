...
class sigesp_apr_c_actspgcuentas
{
    ...
    function sigesp_apr_c_actspgcuentas()
    {
		...
		$ls_nombrearchivo="resultado/".$_SESSION["ls_data_des"]."_actualizar_cuentaspresupuestarias_result_".$ld_fecha.".txt";
    ...
    }//end sigesp_apr_c_actspgcuentas
	//-----------------------------------------------------------------------------------------------------------------------------------
    ...
    function uf_load_cuentas(&$ai_totrows,&$ao_object)
	{
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		$lb_valido=true;
		$ls_sql="SELECT spg_cuenta as spg_cuentaorigen ".
				"  FROM saf_catalogo ".
				" WHERE trim(spg_cuenta)<>'' ".
				"UNION ".
				"SELECT spg_cuenta_act as spg_cuentaorigen ".
				"  FROM saf_activo ".
				" WHERE trim(spg_cuenta_act)<>'' ".
				"UNION ".
				"SELECT spg_cuenta_dep as spg_cuentaorigen ".
				"  FROM saf_activo ".
				" WHERE trim(spg_cuenta_dep)<>'' ".
				"UNION ".
				"SELECT spg_cuenta as spg_cuentaorigen ".
				"  FROM sigesp_cargos ".
				" WHERE trim(spg_cuenta)<>'' ".
				"UNION ".
				"SELECT spg_cuenta as spg_cuentaorigen ".
				...
				" WHERE trim(spg_cuenta)<>'' ".
				"UNION ".
				"SELECT cueprecon as spg_cuentaorigen ".
				"  FROM sno_concepto ".
				" WHERE trim(cueprecon)<>'' ".
				"UNION ".
				"SELECT cueprepatcon as spg_cuentaorigen ".
				"  FROM sno_concepto ".
				...
				"  FROM sno_hconcepto ".
				" WHERE trim(cueprepatcon)<>'' ".
				" GROUP BY spg_cuentaorigen ".
				" ORDER BY spg_cuentaorigen ";
		...
		else
		{
			...
			while(($row=$this->io_sql_origen->fetch_row($io_recordset))&&$lb_valido)
			{
                                ...
                                $ls_cuentadestino="";
				$ls_sccuentao=$this->io_validacion->uf_valida_texto($row["spg_cuentaorigen"],0,25,"");
                                if ($ls_sccuentao!=$ls_sccuentaonext){
                                    $ai_totrows++;
                                    $this->uf_load_cuentadestino($ls_sccuentao,&$ls_cuentadestino);
                                    if ($ls_cuentadestino==""){
                                        $ls_cuentadestino=$ls_sccuentao;
                                    }
                                    $ao_object[$ai_totrows][1]="<input name=txtspgcuentaant".$ai_totrows." type=text id=txtspgcuentaant".$ai_totrows." class=sin-borde size=40 maxlength=25 value='".$ls_sccuentao."' readonly>";
                                    $ao_object[$ai_totrows][2]="<input name=txtspgcuentaact".$ai_totrows." type=text id=txtspgcuentaact".$ai_totrows." class=sin-borde size=50 maxlength=25 value='".$ls_cuentadestino."' readonly><a href=javascript:ue_catalogo_spgcuentas('".$ai_totrows."');><img src='../shared/imagebank/tools20/buscar.gif' alt='Buscar' width='20' height='20' border='0'></a>";
                                    $ls_sccuentaonext=$ls_sccuentao;
                                }
	...
	function uf_insert_cuentas()
	{
		...
		$li_total=$_POST["totrow"];
                ...
                {
                        $ls_sccuentaant=trim($_POST["txtspgcuentaant".$li_i]);
                        $ls_sccuentaact=trim($_POST["txtspgcuentaact".$li_i]);
                        if($ls_sccuentaact!="")
                        {
                                $ls_sql="INSERT INTO apr_presupuestario (spg_cuentaorigen, spg_cuentadestino) VALUES ('".$ls_sccuentaant."','".$ls_sccuentaact."') ";
                                $lb_valido=$this->uf_procesar_sql($ls_sql);
                                if($lb_valido)
                                {
                                        ...
                                        /////////////////////////////////         SEGURIDAD               /////////////////////////////
                                        $ls_evento="INSERT";
                                        $ls_descripcion ="Asocio la cuenta Presupuestaria Origen ".$ls_sccuentaant." con la Cuenta Presupuestaria Destino".$ls_sccuentaact;
