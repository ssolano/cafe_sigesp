	...
	{   
		...
		else
		{
                        ...
                        while(($row=$this->io_sql_origen->fetch_row($io_recordset))&& $lb_valido)
			{
                                    ...
                                    $ai_totrows++;
                                    $ls_cuentadestino="";
                                    $ls_sccuentao=$this->io_validacion->uf_valida_texto($row["spg_cuenta"],0,25,"");
                                    $this->uf_load_cuentadestino($ls_sccuentao,&$ls_cuentadestino);
                                    $ao_object[$ai_totrows][1]="<input name=txtspgcuentaant".$ai_totrows." type=text id=txtspgcuentaant".$ai_totrows." class=sin-borde size=30 maxlength=25 value='".$ls_sccuentao."' style=text-align:center readonly>";
