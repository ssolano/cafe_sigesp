	...
	function uf_insert_conceptos()
	{
		...
		{   
			...
			while($row=$this->io_sql_origen->fetch_row($io_recordset))
			{
				...
				if($ls_codconsep!="")
				{
					$ls_sql="INSERT INTO sep_conceptos (codconsep, denconsep, monconsepe, obsconesp, spg_cuenta) VALUES ".
							"('".$ls_codconsep."','".$ls_denconsep."',".$li_monconsepe.",'".$ls_obsconesp."','".$ls_spg_cuenta."')";
					$li_row=$this->io_sql_destino->execute($ls_sql);
					if($li_row===false)
					{
						$lb_valido=false;
