...
<form name="form1" method="post" action="">
  ...
  <div align="center">
    <table width="565" border="0" cellpadding="0" cellspacing="0" class="formato-blanco">
	  ...
	  <tr>
        <td height="22" align="right">Emitir a </td>
        <td width="113" colspan="2" height="22" align="left"><input type="radio" name="emitir" value="PRO" checked="checked">&nbsp;Proveedor</td>
	  <tr>
        <td height="22">&nbsp;</td>
        <td width="113" colspan="5" height="22" align="left"><input type="radio" name="emitir" value="ALT">&nbsp;Beneficiario Alterno</td>
      </tr>
      <tr>
        <td height="22">&nbsp;</td>
...
if ($ls_operacion=="BUSCAR")
   {
	 ...
	 if($ls_emitir=="PRO")
	 {
	   $ls_sql = "SELECT DISTINCT cxp_solicitudes.cod_pro, rpc_proveedor.nompro, scb_prog_pago.codban,
			...
			$ls_sql = "SELECT DISTINCT cxp_solicitudes.cod_pro, sep_solicitud.nombenalt, rpc_proveedor.nompro, scb_prog_pago.codban,
				           scb_prog_pago.ctaban, scb_banco.nomban, scb_ctabanco.dencta, TRIM(scb_ctabanco.sc_cuenta) as sc_cuenta
				      FROM sep_solicitud, cxp_solicitudes, rpc_proveedor, scb_prog_pago, scb_banco, scb_ctabanco
				     WHERE scb_prog_pago.codemp='".$ls_codemp."'
				       AND scb_prog_pago.numsol like '%".$ls_numsolpag."%'
					   AND cxp_solicitudes.estprosol='S'
				       AND scb_prog_pago.estmov='P'
					   ...
					   AND sep_solicitud.cod_pro=cxp_solicitudes.cod_pro
					   AND sep_solicitud.consol=cxp_solicitudes.consol
					   AND cxp_solicitudes.numsol=scb_prog_pago.numsol
				     ORDER BY cxp_solicitudes.cod_pro, rpc_proveedor.nompro, scb_prog_pago.codban,
					          scb_prog_pago.ctaban, scb_banco.nomban, scb_ctabanco.dencta, sc_cuenta ASC";
			    ...
			    while(!$rs_data->EOF)
				     {
					   ...
					   if($ls_emitir=="PRO")
					   {
					   	 $ls_nompro = $rs_data->fields["nompro"];
					   			...
					   			$ls_nompro = $rs_data->fields["nombenalt"];
								if ($ls_nompro == '' && $ls_emitir=="ALT" )
								   {
									 $ls_nompro = $rs_data->fields["nompro"];
									 $io_msg->message("La operación no puede ser realizada, no existe beneficiario alterno para el proveedor $ls_nompro, para la emisión del cheque se usara el nombre del proveedor");
								   }
					        }
