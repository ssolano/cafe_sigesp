...
/*	if (array_key_exists("cuenta",$_GET))
	...
	if (array_key_exists("programatico",$_GET))
    {
	 $ls_programatico = $_GET["programatico"];
	}
	else
	{
	 $ls_programatico = "";
...
if($ls_operacion=="GUARDARPRE")
{
	...
	else
	{
	    ...
	    $ld_fecha=$fun->uf_convertirdatetobd($ld_fecha);
		...
		$lb_valido=$in_classcmp->uf_guardar_automatico($ls_comprobante,$ld_fecha,$ls_proccomp,$ls_desccomp,&$ls_prov,&$ls_bene,$ls_tipo,2,0,$ls_codfuefincab,$ls_coduniadm);
		if(!$lb_valido)
		{
		...
		{
		    /*$ld_fecdb=$fun->uf_formatovalidofecha($ld_fecdb);
			$ld_fecdb=$fun->uf_convertirdatetobd($ld_fecdb);
		...
		$arr_cmp["tipo_comp"]  =1;
		//$arr_cmp["codfuefin"]  =$ls_codfuefin;
		if($lb_valido)
		{
...
<table width="603" border="0" align="center" cellpadding="0" cellspacing="0" class="formato-blanco">
  <tr>
   <td height="22" colspan="2" class="titulo-celda">ENTRADA DE  SOLICITUD DE<br>
MODIFICACION PRESUPUESTARIA POR <? print strtoupper($ls_titulo); ?> </td>
  ...
  <tr>
    <td width="142" height="22" align="right">Documento</td>
    <td width="439"><input name="txtdocumento" type="text" id="txtdocumento" style="text-align:center" onBlur="javascript:valid_cmp(this);" size="22" maxlength="15" value="<?php print $ls_documento;?>" <? print $ls_read;?>></td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td height="22"><div align="right">Fuente de Financiamiento</div></td>
        <td><input name="txtcodfuefin" type="text" id="txtcodfuefin" readonly="true" value="<?php print $ls_codfuefindet ;?>" size="22" style="text-align:center">
            ...
            <input name="txtdenfuefin" type="text" class="sin-borde" id="txtdenfuefin" style="text-align:left" value="<?php print $ls_denfuefindet ?>" size="53" maxlength="254"></td>
    </tr>
  <tr>
...
<script language="JavaScript">
  function aceptar_presupuestario()
  {
	...
	if(uf_valida_campos()&&(lb_valido))
	{
		...
		f.action="sigesp_w_regdt_traspaso.php";
		f.submit();
	}
	else
	{
...
function rellenar_cad(cadena,longitud,campo)
{
 ...
 function catalogo_cuentasSPG()
 {
	   ...
	   operacion =f.txtoperacion.value
	   procede = "SPGTRA";
	   if(estmodest==1)
	   {
		   if((codest1!="")&&(codest2!="")&&(codest3!=""))
		   {
			   pagina="sigesp_cat_ctasspg.php?codestpro1="+codest1+"&hicodest2="+codest2+"&hicodest3="+codest3+"&estcla="+estcla+"&procede="+procede+"&opeaumdis="+operacion;
		...
		else
		{
		   ...
		   if((codest1!="")&&(codest2!="")&&(codest3!="")&&(codest4!="")&&(codest5!=""))
		   {
			   ...
			   +"&hicodest5="+codest5+"&estcla="+estcla+"&procede="+procede+"&opeaumdis="+operacion;
