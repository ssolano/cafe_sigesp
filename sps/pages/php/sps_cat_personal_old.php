...
{ 
  //$lb_hay = $lo_dao->getPersonal("ORDER BY p.codper ASC",$_GET["cedper"],$_GET["nomper"],$_GET["apeper"],$_GET["codnom"],$la_datos);
  $lb_hay = $lo_dao->getPersonal("ORDER BY p.codper ASC","7416573","","","",$la_datos);
  ...
  else
  {
    ...
    $ls_salida   .= '<table id="data_grid" width="600" class="fondo-tabla" cellpadding="0" cellspacing="1">';

	for($i=1; $i<=$li_registros; $i++)
	{
	  $ls_salida .='<tr class="celdas-blancas">';
	  $ls_salida .='<td align="center" width="90">'.$la_datos["codper"][$i].'</td>';
	  $ls_salida .='<td align="center" width="150">'.$la_datos["nomper"][$i].'</td>';
	  $ls_salida .='<td align="center" width="148">'.$la_datos["apeper"][$i].'</td>';
	  $ls_salida .='<td align="center" width="60">'.$la_datos["codnom"][$i].'</td>';
	  $ls_salida .='<td align="center" width="152">'.$la_datos["desnom"][$i].'</td>';
	  $ls_salida .='</tr>';
    }
    $ls_salida.= '</table>';
  }
...
else
{
  ...
  $lb_hay = $lo_dao->getPersonal("ORDER BY $sort_col $sort_dir LIMIT $offset,$pagesize",$ls_cedper,$ls_nomper,$ls_apeper,$ls_codnom,$la_datos);
  ...
  if ($lb_hay)
  {
	...
	for ($i=1; $i<=$li_registros; $i++)
    {
      ...
      $ls_salida .='<tr class="celdas-blancas">';
	  $ls_salida .='<td align="center" width="200"><a href="javascript:ue_seleccionar('.$la_newarr.');">'.$la_datos["codper"][$i].'</a></td>';
	  $ls_salida .='</tr>';
    }
