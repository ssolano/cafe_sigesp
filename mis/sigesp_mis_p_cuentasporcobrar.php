<?Php
	...
	$oi_fun_integrador->uf_load_seguridad("MIS","sigesp_mis_p_cuentasporcobrar.php",$ls_permisos,$la_seguridad,$la_permisos);
	//////////////////////////////////////////////         SEGURIDAD               /////////////////////////////////////////////
...
<html>
<head>
<title>Registro de Cuentas por Cobrar - Cr&eacute;ditos</title>
...
<?php
	...
	switch ($ls_operacion) 
	{
		...
		case "PROCESAR":
			for($li_i=1;$li_i<=$li_totrows;$li_i++)
			{
				if(array_key_exists("chksel".$li_i,$_POST))
				{
					...
					if($lb_valido)
					{
						$lb_valido=$in_class->uf_procesar_cuentasporcobrar($ls_archivo,$ls_codemp,$ls_comprobante,$ls_fecha,$ls_procede,$ls_codban,
		...
		case "BUSCAR":
	    	$in_class_contabiliza->uf_select_cuentasporcobrar($lo_object,$li_totrows);
			if($li_totrows==0)
			{
				$li_totrows=1;
				uf_agregarlineablanca($lo_object,$li_totrows);
...
function ue_procesar()
{
	...
	if(li_ejecutar==1)
	{
		...
		if(lb_valido)
		{
			...
			f.action="sigesp_mis_p_cuentasporcobrar.php";
			f.submit();
		}
		else
		{
