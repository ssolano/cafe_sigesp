<?php
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_cabecera1($as_cedper,$as_nomper,$as_descar,&$io_cabecera,&$io_pdf, $as_desubifis, $ai_sueper)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$li_tm=$io_pdf->getTextWidth(9,$as_desnom);
		$tm=306-($li_tm/2);
		$li_ubi=$io_pdf->getTextWidth(9,$as_desubifis);
		$tm_ubi=306-($li_ubi/2);
		$io_pdf->addText($tm,715,9,$as_desnom); // Agregar el título
		$io_pdf->addText($tm_ubi,695,9,"<b><i>".$as_desubifis."</i></b>"); // Agregar ubicacion fisica
		$io_pdf->addText(40,695,'10','______________________________________________________________________________________________');
		$io_pdf->ezSetDy(-11);
		$la_data=array(array('nombre'=>'Apellidos y Nombres:', 'cedula'=>'Cédula:', 'cargo'=>'Cargo:', 'sueldomensual'=>'Sueldo Mensual:'  ));
		$la_columna=array('nombre'=>'','cedula'=>'','cargo'=>'','sueldomensual'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						...
						'cols'=>array('nombre'=>array('justification'=>'left','width'=>200), // Justificación y ancho de la columna
						 			   ...
						 			   'cargo'=>array('justification'=>'left','width'=>200),
                                                                            'sueldomensual'=>array('justification'=>'left','width'=>100)));
		...
		$la_data=array(array('nombre'=>$as_nomper, 'cedula'=>$as_cedper, 'cargo'=>substr($as_descar,0,100),'sueldomensual'=> number_format($ai_sueper,2,',','.')." Bs."));
		$la_columna=array('nombre'=>'','cedula'=>'','cargo'=>'', 'sueldomensual'=>'');
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 'xPos'=>315,
	...
	//--------------------------------------------------------------------------------------------------------------------------------
	function uf_print_cabecera2($as_cedper,$as_nomper,$as_descar,&$io_cabecera,&$io_pdf, $as_desubifis, $ai_sueper)
	{
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		$io_pdf->addText($tm_ubi,275,9,"<b><i>".$as_desubifis."</i></b>"); // Agregar ubicacion fisica
		$io_pdf->addText(40,275,'10','______________________________________________________________________________________________');
		$io_pdf->ezSetDy(-175);
		$la_data=array(array('nombre'=>'Apellidos y Nombres:', 'cedula'=>'Cédula:', 'cargo'=>'Cargo:', 'sueldomensual'=>'Sueldo Mensual:'  ));
		$la_columna=array('nombre'=>'','cedula'=>'','cargo'=>'','sueldomensual'=>'');
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('nombre'=>array('justification'=>'left','width'=>200), // Justificación y ancho de la columna
						 			   'cedula'=>array('justification'=>'center','width'=>50), // Justificación y ancho de la columna
						 			   'cargo'=>array('justification'=>'left','width'=>200),
									   'sueldomensual'=>array('justification'=>'left','width'=>100))); // Justificación y ancho de la columna*/
	...
	function uf_print_detalle($la_data,&$io_pdf)
	{
		...
		$la_config=array('showHeadings'=>0, // Mostrar encabezados
						 ...
						 'cols'=>array('denomasig'=>array('justification'=>'left','width'=>130), // Justificación y ancho de la columna
									   ...
									   'unidaddedu'=>array('justification'=>'center','width'=>50), // Justificación y ancho de la columna
	...
	{
		case "0":
			require_once("sigesp_snorh_class_report_camb.php");
			$io_report=new sigesp_snorh_class_report();
			$ls_bolivares ="Bs.";
			break;

		case "1":
			require_once("sigesp_snorh_class_reportbsf_camb.php");
	...
	else // Imprimimos el reporte
	{
		...
		while((!$io_report->rs_data->EOF)&&($lb_valido))
		{
			...
			if($li_reg==1)
			{
				uf_print_cabecera1($ls_cedper,$ls_nomper,$ls_descar,&$io_cabecera,&$io_pdf, $ls_desubifis, $li_sueper); // Imprimimos la cabecera del registro
			}
			else
			{
				uf_print_cabecera2($ls_cedper,$ls_nomper,$ls_descar,&$io_cabecera,&$io_pdf, $ls_desubifis,$li_sueper); // Imprimimos la cabecera del registro
			}
			$ls_codperi="";
			if($ls_consolidar=="0")
			{
			...
			if($lb_valido)
			{
				...
				else// No utiliza adelanto de quincena
				{
					for($li_s=1;$li_s<=$li_totrow_det;$li_s++) 
					{
						//$ls_unidad=$io_report->DS_detalle->data["unidad"][$li_s]; //CAMB	
						//$ls_denunidad=$io_report->DS_detalle->data["denunidad"][$li_s]; //CAMB
						$ls_tipsal=rtrim($io_report->DS_detalle->data["tipsal"][$li_s]);
						if(($ls_tipsal=="A") || ($ls_tipsal=="V1") || ($ls_tipsal=="V2") || ($ls_tipsal=="R")) // Buscamos las asignaciones
						{
							$li_asig=$li_asig+1;
							...
							$li_toting=$li_toting+abs($io_report->DS_detalle->data["valsal"][$li_s]);
							$li_valsal=$io_fun_nomina->uf_formatonumerico(abs($io_report->DS_detalle->data["valsal"][$li_s]));
							$la_data_a[$li_asig]=array('denominacion'=>$ls_nomcon,'valor'=>$li_valsal,'unidad'=>$ls_unidad,'denunidad'=>$ls_denunidad);
						...
						else // Buscamos las deducciones y aportes
						{
							...
							$li_totded=$li_totded+abs($io_report->DS_detalle->data["valsal"][$li_s]);
							$li_valsal=$io_fun_nomina->uf_formatonumerico(abs($io_report->DS_detalle->data["valsal"][$li_s]));
							$la_data_d[$li_dedu]=array('denominacion'=>$ls_nomcon,'valor'=>$li_valsal,'unidad'=>$ls_unidad,'denunidad'=>$ls_denunidad);
						}
					}
				}
				if($li_asig<=$li_dedu)
				...
				for($li_s=1;$li_s<=$li_total;$li_s++) 
				{
					...
					if($li_s<=$li_asig)
					{
						...
						if ($la_data_a[$li_s]["unidad"] != 0):
							$la_valores["unidad"] =  $la_data_a[$li_s]["unidad"] . " " . $la_data_a[$li_s]["denunidad"]; //Unidad CAMB
						else:
							$la_valores["unidad"] =  ''; //Unidad CAMB
						endif;
					}
					if($li_s<=$li_dedu)
					{
						...
						if ($la_data_d[$li_s]["unidad"] != 0):
							$la_valores["unidaddedu"] =  $la_data_d[$li_s]["unidad"] . " " . $la_data_d[$li_s]["denunidad"]; //Unidad CAMB
						else:
							$la_valores["unidaddedu"] =  ''; //Unidad CAMB
						endif;
					}
					$la_data[$li_s]=$la_valores;
				}
//echo "<pre>".print_r($la_valores, true)."</pre>"; die();
