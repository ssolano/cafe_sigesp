<?php
	...
	else // Imprimimos el reporte
	{
		...
		$lo_hoja->write(4, 2, "Apellidos y Nombres",$lo_titulo);
                $lo_hoja->write(4, 3, "Género",$lo_titulo);
                $lo_hoja->write(4, 4, "Tipo Personal",$lo_titulo);
		$lo_hoja->write(4, 5, "Cargo",$lo_titulo);
		$lo_hoja->write(4, 6, "Unidad Adscripción",$lo_titulo);
		$lo_hoja->write(4, 7, $ls_titulo,$lo_titulo);
		$lo_hoja->write(4, 8, "Fecha Ingreso",$lo_titulo);
		$lo_hoja->write(4, 9, "Cuenta Bancaria",$lo_titulo);
		$lo_hoja->write(4, 10, "Ubicación Física",$lo_titulo);
		$lo_hoja->write(4, 11, "Estado",$lo_titulo);
		$lo_hoja->write(4, 12, "Municipio",$lo_titulo);
		$lo_hoja->write(4, 13, "Parroquia",$lo_titulo);
		$lo_hoja->write(4, 14, "Salario",$lo_titulo);
		$lo_hoja->write(4, 15, "Fecha de Ingreso a Nómina",$lo_titulo);
		$lo_hoja->write(4, 16, "Fecha de Culminación de Contrato",$lo_titulo);
		$li_col=16;
		...
		while(!$io_report->rs_data->EOF)
		{
			...
			$ls_cedper=$io_report->rs_data->fields["cedper"];
                        if ($io_report->rs_data->fields["sexper"]=='F'){
                            $ls_sexper='FEMENINO';
                        }else{
                            $ls_sexper='MASCULINO';
                        }
			...
			$lo_hoja->write($li_row, 14, $ld_sueper, $lo_dataright);
			$lo_hoja->write($li_row, 15, $ld_fecingnom, $lo_datacenter);
			$lo_hoja->write($li_row, 16, $ld_feculcont, $lo_datacenter);
			$li_col=16;
