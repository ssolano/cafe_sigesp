	...
	function uf_print_deducciones()
   	{
		...
		{
			$ls_sql="SELECT codded,dended,formula,porded,monded,sc_cuenta,islr,iva,estretmun,retaposol ".
	...
	function uf_print_solicitudespago()
	{
		...
		else
		{
			...
			while($row=$io_sql->fetch_row($rs_data))
			{
				...
				$ls_nombre=utf8_encode($row["nombre"]);
				$ls_rifproben = utf8_encode($row["rifproben"]);
				...
				switch ($ls_tipo)
				{
					case "":
						...
						print "'$ls_estatus','$ls_tipo_destino','$li_i','$ls_rifproben');\">".$ls_numsol."</a></td>";
